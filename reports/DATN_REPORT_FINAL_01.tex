...
Thiết lập đường truyền kết nối trực tiếp PostgreSQL bảo mật TLS/SSL từ Tableau Desktop tới Supabase Connection Pooler. Xây dựng bộ 3 Dashboard tương tác chuyên sâu:
\begin{itemize}[noitemsep]
...
\begin{enumerate}[label=\arabic*., leftmargin=*]
    ...
    \begin{itemize}[noitemsep]
        ...
        \item \textbf{Estimated Financial Loss (\$)}: $\text{Tổn thất sản lượng} \times \text{Đơn giá điện FiT}$.
    \end{itemize}
...
\newpage
\chapter{Phát hiện Dị thường và Làm sạch Dữ liệu}
...
0        v4_train  1550856        114       49       39       10   
1          v4_val   723114        114       49       39       10   
2         v4_test   510468        114       49       39       10   
3  v4_development  2273970        113       49       39       10   
4    fold_1_train   233244        115       49       39       10   
5      fold_1_val   606105        115       49       39       10   
6    fold_2_train   839349        115       49       39       10   
7      fold_2_val   711507        115       49       39       10   
8    fold_3_train  1550856        115       49       39       10   
9      fold_3_val   723114        115       49       39       10   
...
Dự án đã hoàn thành toàn diện các mục tiêu nghiên cứu và kỹ thuật đặt ra, giải quyết trọn vẹn bài toán từ khâu thu thập, tiền xử lý, lưu trữ, phân tích bất thường cho đến dự báo sản lượng và trực quan hóa dữ liệu vận hành hệ thống điện mặt trời:

\begin{itemize}
    \item \textbf{Xây dựng kiến trúc dữ liệu vững chắc:} Thiết kế và vận hành thành công Data Warehouse trên nền tảng Supabase (PostgreSQL) theo mô hình Galaxy Schema, tích hợp cơ chế Connection Pooling, Indexing tối ưu và Materialized Views, đáp ứng khả năng truy vấn phân tích đa chiều với hiệu năng cao.
    \item \textbf{Chuẩn hóa và xử lý dữ liệu chuỗi thời gian:} Triển khai hoàn chỉnh pipeline ETL/ELT tự động với thuật toán Causal Cascade Imputation, đảm bảo xử lý triệt để dữ liệu khuyết thiếu mà không gây hiện tượng rò rỉ thông tin tương lai (data leakage), đồng thời đồng bộ hóa thành công dữ liệu đa tần số thời gian (15 phút và 1 giờ).
    \item \textbf{Phân tích bất thường và dự báo sản lượng chuyên sâu:} Ứng dụng mô hình phân lớp lai GMM--IF (Gaussian Mixture Model -- Isolation Forest) để nhận diện và phân loại chính xác các dạng bất thường kỹ thuật (như suy hao do nhiệt, che khuất, hoặc lỗi cảm biến). Phát triển mô hình hồi quy LightGBM kết hợp tối ưu siêu tham số Optuna và giải thích mô hình bằng SHAP (SHapley Additive exPlanations), đạt độ chính xác cao trong việc dự báo công suất phát điện.
    \item \textbf{Hỗ trợ ra quyết định dựa trên dữ liệu:} Triển khai hệ thống Dashboard quản trị hoàn chỉnh trên Tableau Desktop với cấu trúc UI/UX theo bảng màu ngữ nghĩa và các trường tính toán phức hợp, giúp doanh nghiệp chuyển đổi từ phương thức bảo trì thụ động (reactive maintenance) sang giám sát chủ động theo thời gian thực (data-driven proactive monitoring).
\end{itemize}

\section{Khuyến nghị Chiến lược (Actionable Recommendations)}
\label{sec:recommendations}
Dựa trên kết quả phân tích thực nghiệm từ dữ liệu vận hành, nhóm đề xuất các giải pháp kỹ thuật và vận hành cụ thể nhằm tối ưu hóa hiệu suất (Performance Ratio -- PR) của hệ thống:

\begin{enumerate}
    \item \textbf{Kiểm soát và giảm thiểu tổn thất do nhiệt độ (Thermal Loss Mitigation):} Kết quả mô hình SHAP chỉ ra nhiệt độ bề mặt tấm pin (Module Temperature) là một trong những yếu tố làm suy giảm hiệu suất mạnh nhất khi bức xạ đạt đỉnh. Đơn vị vận hành cần thiết lập cơ chế cảnh báo tự động khi chênh lệch nhiệt độ vượt ngưỡng an toàn, đồng thời cân nhắc lắp đặt hệ thống phun sương làm mát bề mặt cục bộ vào các khung giờ nắng gắt nhằm duy trì dải nhiệt độ tối ưu cho pin quang điện.
    \item \textbf{Chuẩn hóa quy trình kiểm tra Inverter và tái hiệu chuẩn cảm biến:} Hiện tượng tiêu thụ công suất ký sinh ban đêm (parasitic power consumption) và độ nhiễu tín hiệu telemetry cần được xử lý thông qua quy trình kiểm định Inverter định kỳ. Cần thực hiện tái hiệu chuẩn (recalibration) định kỳ cảm biến bức xạ (pyranometer) để tránh hiện tượng trôi dạt cảm biến (sensor drift), đảm bảo tính chính xác của dữ liệu đầu vào cho hệ thống giám sát.
    \item \textbf{Tối ưu hóa lịch trình vệ sinh bề mặt tấm pin (Dynamic Soiling Mitigation):} Thay vì áp dụng lịch vệ sinh định kỳ cố định (vốn gây lãng phí chi phí nhân công và nước), doanh nghiệp nên ứng dụng chênh lệch giữa công suất dự báo của mô hình AI và công suất phát thực tế để xác định mức độ tích tụ bụi bẩn (soiling ratio). Lịch bảo trì sẽ chỉ được kích hoạt khi chi phí tổn thất sản lượng điện vượt qua chi phí vệ sinh thực tế.
\end{enumerate}

\section{Hạn chế của Dự án}
\label{sec:limitations}
Mặc dù đã đạt được các kết quả khả quan, đề tài vẫn tồn tại một số hạn chế nhất định do điều kiện thực nghiệm:

\begin{itemize}
    \item \textbf{Phạm vi dữ liệu đo đạc chuyên sâu:} Hệ thống dữ liệu hiện tại chưa tích hợp cảm biến đo lường chỉ số bám bụi chuyên dụng (Soiling Sensor) hoặc hình ảnh nhiệt hồng ngoại (IR Thermography) từ thiết bị bay không người lái (UAV), dẫn đến việc chẩn đoán các lỗi cục bộ (hot-spots, nứt tế bào cell, hỏng bypass diode) còn phụ thuộc gián tiếp vào suy luận từ thông số dòng/áp.
    \item \textbf{Độ trễ trong kiến trúc xử lý:} Quy trình xử lý hiện tại chủ yếu vận hành theo cơ chế Batch Processing định kỳ theo chu kỳ giờ/ngày. Hệ thống chưa hỗ trợ xử lý phân tán luồng dữ liệu liên tục (Streaming Pipeline) ở mức độ mili-giây để phản hồi tức thời các sự cố lưới điện (grid transient events).
    \item \textbf{Hạn chế về hạ tầng tính toán cho Deep Learning:} Do giới hạn về tài nguyên phần cứng máy trạm, dự án tập trung tối ưu hóa các thuật toán dạng Tree-based Ensemble (LightGBM) thay vì huấn luyện các kiến trúc mạng nơ-ron học sâu phức tạp (như Temporal Fusion Transformer hay Informer) đòi hỏi khối lượng dữ liệu khổng lồ và năng lực GPU chuyên dụng để hội tụ.
\end{itemize}

\section{Hướng phát triển trong tương lai}
\label{sec:future_work}

\subsection{Nâng cấp Hệ thống Phân tích Dự đoán}
Nhằm nâng cao độ tin cậy và khả năng ứng dụng thực tiễn, hệ thống dự báo sẽ được mở rộng theo các hướng:
\begin{itemize}
    \item \textbf{Tích hợp các kiến trúc Deep Learning chuỗi thời gian:} Nghiên cứu và triển khai các mô hình học sâu chuyên biệt cho dữ liệu tuần tự dài hạn như Temporal Fusion Transformer (TFT), N-BEATS hoặc PatchTST nhằm mô hình hóa chính xác hơn các phụ thuộc phi tuyến tính phức tạp và đưa ra dự báo đa bước thời gian (multi-horizon forecasting) đi kèm khoảng tin cậy (uncertainty quantification).
    \item \textbf{Kết hợp dữ liệu khí tượng vệ tinh và Nowcasting:} Tích hợp nguồn cấp dữ liệu dự báo thời tiết số (Numerical Weather Prediction -- NWP) và ảnh vệ tinh thời gian thực để nâng cao năng lực dự báo ngắn hạn (nowcasting 15--60 phút), hỗ trợ hiệu quả cho bài toán điều độ công suất hòa lưới điện quốc gia.
\end{itemize}

\subsection{Tự động hóa và Đưa lên Nền tảng Đám mây (Cloud \& Automation)}
\label{sec:future_work_cloud}
Chuyển đổi toàn diện hệ thống từ môi trường kết hợp (hybrid) sang hạ tầng điện toán đám mây hoàn chỉnh:
\begin{itemize}
    \item \textbf{Xây dựng Streaming Pipeline trên nền tảng Cloud:} Chuyển dịch hạ tầng sang các dịch vụ như AWS (Kinesis, Lambda, Glue, Redshift) hoặc GCP (Pub/Sub, Dataflow, BigQuery) để thu nhận và xử lý dòng dữ liệu streaming từ hàng nghìn inverter và cảm biến IoT theo thời gian thực.
    \item \textbf{Tự động hóa quy trình MLOps và DataOps:} Ứng dụng Apache Airflow để điều phối workflow dữ liệu tự động, sử dụng MLflow cho việc quản lý vòng đời mô hình (Model Registry), và cấu hình cơ chế tự động huấn luyện lại mô hình (Continuous Training) khi hệ thống phát hiện hiện tượng trôi dạt dữ liệu (Data Drift) hoặc trôi dạt khái niệm (Concept Drift).
\end{itemize}
\newpage
% ====================================================================
...
Dự án tốt nghiệp được triển khai theo khung phương pháp Agile/Scrum qua 14 tuần làm việc liên tục (8 Sprints), bắt đầu từ khâu khảo sát dữ liệu đến đóng gói báo cáo và hệ thống Dashboard.
...
\toprule
\textbf{Sprint} & \textbf{Thời Gian} & \textbf{Mục Tiêu Trọng Tâm} & \textbf{Sản Phẩm Đầu Ra (Deliverables)} \\
\midrule
Sprint 1 & Tuần 1 -- 2 & Khởi tạo dự án, khảo sát bài toán & Kế hoạch đề tài, Git Repo, khảo sát UNISOLAR. \\
Sprint 2 & Tuần 3 -- 4 & Thu thập dữ liệu \& Thu thập Open-Meteo API & 2,73 triệu dòng dữ liệu thô, Pipeline nạp API thời tiết. \\
Sprint 3 & Tuần 5 -- 6 & Thiết kế Kiến trúc Kho dữ liệu & DDL Supabase PostgreSQL, Lược đồ Galaxy Schema. \\
Sprint 4 & Tuần 7 -- 8 & Tiền xử lý, Imputation \& Lọc nhiễu & Pipeline ETL v3/v4, Điền khuyết nhân quả 6 mức. \\
Sprint 5 & Tuần 9 -- 10 & Phân lớp dị thường GMM--IF \& BI Mart & Bảng \texttt{gmm\_if\_outlier}, 6 mã lý do kỹ thuật, BI Mart. \\
Sprint 6 & Tuần 11 & Thiết kế Dashboard Tableau tương tác & Bộ 3 Dashboard Tableau hoàn chỉnh (main\_fixed\_01.twb). \\
Sprint 7 & Tuần 12 -- 13 & Huấn luyện mô hình Machine Learning & Pipeline LightGBM, 50 Trials Optuna, Phân tích SHAP. \\
Sprint 8 & Tuần 14 & Đóng gói ứng dụng, Báo cáo Final & Streamlit App, Báo cáo tốt nghiệp hoàn chỉnh. \\
\bottomrule
\end{tabularx}
\caption{Lộ trình 14 tuần thực hiện dự án theo khung phương pháp Agile/Scrum.}
...
\bottomrule
\end{tabularx}
\caption{Bảng phân công nhiệm vụ và khối lượng hoàn thành của các thành viên trong nhóm.}
...
\begin{figure}[H]
    ...
    \caption[Vòng đời học máy, bản nhãn gốc tiếng Anh]{Vòng đời học máy, giữ nguyên nhãn tiếng Anh của tài liệu gốc. Nguồn: \textit{Data Version Control}~\cite{aivietnam2025dvc}.}
...
\bottomrule
\end{tabularx}
\caption{Bảng đối chiếu thuật ngữ kỹ thuật MLOps Anh -- Việt sử dụng trong dự án.}
...
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{3.4cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
\toprule
...
\bottomrule
\end{tabularx}
\caption{Bảng ma trận tra cứu chi tiết 6 mã dị thường kỹ thuật GMM--IF và quy trình xử lý.}
