...
\bottomrule
\end{tabular}
\caption[Ba nhóm đặc trưng sinh tại notebook \texttt{03\_1\_features\_time.ipynb}]{Ba nhóm đặc trưng sinh tại notebook \texttt{03\_1\_features\_time.ipynb}. Cột cuối ghi
tham số thực tế khai báo trong notebook.}
\label{tab:dac_trung_thoi_gian}
\end{table}
...
\bottomrule
\end{tabular}
\caption[Hệ số hiệu chỉnh trời quang \texttt{cs\_factor} theo từng fold]{Hệ số hiệu chỉnh trời quang \texttt{cs\_factor} theo từng fold. Đọc từ ô kết quả của
notebook \texttt{03\_2\_features\_spatial.ipynb}.}
\label{tab:cs_factor}
\end{table}
...
\bottomrule
\end{tabular}
\caption[Số điểm bị ngưỡng \texttt{CLIP\_CSI} cắt]{Số điểm bị ngưỡng \texttt{CLIP\_CSI} cắt. Đọc từ tệp kết quả
\texttt{05b\_thuc\_nghiem\_tham\_so/thuc\_nghiem\_clip\_csi.csv}.}
\label{tab:clip_csi}
\end{table}
...
\bottomrule
\end{tabular}
\caption[Miền tìm kiếm siêu tham số (\textit{hyperparameter}) khai báo cho Optuna]{Miền tìm kiếm siêu tham số (\textit{hyperparameter}) khai báo cho Optuna. Đọc từ hàm \texttt{tham\_so\_co\_ban} trong
notebook \texttt{06\_1\_train\_mae.ipynb}.}
\label{tab:mien_optuna}
\end{table}
...
$28{,}32\%$, trung vị $18{,}11\%$; ở H4 khoảng này là $15{,}11\%$ đến $35{,}89\%$, trung vị
...
\bottomrule
\end{tabular}
\caption[Ba dự báo được mổ xẻ bằng SHAP]{Ba dự báo được mổ xẻ. Đọc từ \texttt{08\_explain/local\_shap\_cases.csv}.}
\label{tab:shap_local}
\end{table}
...
\begin{itemize}
    ...
    tầm H1, và $15{,}11\%$ tới $35{,}89\%$ ở tầm H4. Mô hình dùng chung chưa mô tả hết phần khác
...
Hình~\ref{fig:ml_lifecycle} ở Mục~$7.1.4$ dùng bản đã chuyển nhãn sang tiếng Việt. Phụ lục này đặt bản giữ nguyên nhãn tiếng Anh của tài liệu gốc, để đối chiếu khi cần tra lại thuật ngữ.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.72\textwidth]{sodo_ml_lifecycle_en.pdf}
