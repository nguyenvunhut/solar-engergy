...
\textbf{Trưởng nhóm (Team Leader):} Điều phối tổng thể, Thiết kế kiến trúc Pipeline ETL/MLOps, Nghiên cứu thuật toán phân lớp lai GMM--IF, Huấn luyện mô hình LightGBM/Optuna/SHAP, Biên tập báo cáo tổng thể. & 
$100\%$ \\
\midrule
\textbf{Nguyễn Vũ Nhựt} & PS44442 & 
Thiết kế Kiến trúc Kho Dữ liệu (Supabase DWH), Viết DDL bảng Fact/Dim theo Galaxy Schema, Cấu hình Connection Pooling, Tối ưu hóa chỉ mục (Indexes) và truy vấn SQL. & 
$100\%$ \\
\midrule
\textbf{Nguyễn Văn Sỹ} & PS43671 & 
Phân tích Khám phá Dữ liệu (EDA), Kiểm định phân phối đơn biến và đa biến, Khảo sát tương quan bức xạ -- nhiệt độ, Tổng hợp các biểu đồ phân phối mẫu hình phát điện. & 
$100\%$ \\
\midrule
\textbf{Lê Công Toàn} & PS44145 & 
Xây dựng module Tiền xử lý dữ liệu (Transform), Triển khai thuật toán Điền khuyết nhân quả (Causal Cascade Imputation), Xử lý đồng bộ độ phân giải thời gian (15m vs 1h). & 
$100\%$ \\
\midrule
\textbf{Nguyễn Xuân Hùng} & PS44527 & 
Phát triển Hệ thống Trực quan hóa Quản trị trên Tableau Desktop, Thiết kế giao diện UI/UX theo bảng màu ngữ nghĩa, Xây dựng các trường tính toán (Calculated Fields) phức hợp. & 
$100\%$ \\
\bottomrule
\end{tabularx}
...
% ====================================================================
%  PHỤ LỤC B: CẤU TRÚC GÓI MÃ NGUỒN VÀ HỆ THỐNG
% ====================================================================
\chapter*{Phụ lục B: Cấu trúc Gói Mã nguồn và Hệ thống}
\addcontentsline{toc}{chapter}{Phụ lục B: Cấu trúc Gói Mã nguồn và Hệ thống}
\label{app:code}
...
\small
\renewcommand{\arraystretch}{1.3}
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.8cm} >{\raggedright\arraybackslash}p{3.4cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X @{}}
\toprule
\textbf{Mã Kỹ Thuật} & \textbf{Điều Kiện Kích Hoạt} & \textbf{Bản Chất Sự Cố / Vật Lý} & \textbf{Hành Động Khuyến Nghị} \\
\midrule
\texttt{zero\_drift} & $GHI = 0\,\text{W/m}^2$, $h \le 0^\circ$, $0 < E \le 0{,}025\,\text{kWh}$ & Biến dòng CT bị trôi điểm cân bằng điện áp 0V ban đêm. & Tầng ETL tự động ép về $0$; Lên lịch hiệu chuẩn biến dòng CT. \\
\midrule
\texttt{night\_leakage} & $GHI = 0\,\text{W/m}^2$, $h \le 0^\circ$, $E > 0{,}025\,\text{kWh}$ & Dòng rò cảm ứng hoặc vi xử lý Inverter tiêu thụ điện tự dùng. & Kiểm tra an toàn cách điện tủ phân phối AC và chống rò. \\
\midrule
\texttt{underperformance} & $GHI \ge 500\,\text{W/m}^2$, $E < 0{,}5 \times E_{\text{expected}}$ & Hỏng diode bypass, đứt cầu chì DC 1 chuỗi, hoặc cell pin bị rạn nứt. & Cử kỹ sư dùng camera nhiệt hồng ngoại quét tìm chuỗi pin lỗi. \\
\midrule
\texttt{inverter\_zero\_output} & $GHI \ge 700\,\text{W/m}^2$, $E = 0\,\text{kWh}$ & Biến tần ngắt mạch khẩn cấp do quá áp lưới ($>253\text{V}$) hoặc quá nhiệt. & Kiểm tra rơ-le bảo vệ điện áp lưới và hệ thống quạt tản nhiệt. \\
\midrule
\texttt{thermal\_derating} & $T_{\text{amb}} \ge 40^\circ\text{C}$, $GHI \ge 800$, $0{,}6 \le E/E_{\text{exp}} \le 0{,}85$ & Biến tần tự động giảm tải để bảo vệ linh kiện bán dẫn công suất IGBT. & Lắp tấm chắn nắng phản quang; vệ sinh lưới tản nhiệt biến tần. \\
\midrule
\texttt{inverter\_clipping} & $GHI \ge 850\,\text{W/m}^2$, $E \approx P_{\text{AC, max}}$ (đỉnh phẳng) & Công suất DC vượt định mức AC ($ILR = 1{,}25$). & Tối ưu kinh tế thiết kế theo chuẩn CEC; không cần can thiệp kỹ thuật. \\
\bottomrule
\end{tabularx}
