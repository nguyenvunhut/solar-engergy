...
\begin{center}
    ...
    \end{flushleft}
    \vfill
    {\normalsize \textbf{TP. HỒ CHÍ MINH, THÁNG 5 NĂM 2026}}
    \vspace{0.2cm} % Tạo một khoảng đệm nhỏ an toàn với đường viền dưới
\end{center}
\end{titlepage}
...
\vspace*{-15pt} % Giữ khoảng cách dọc khít với tiêu đề

% Dịch toàn bộ khối bảng sang trái một khoảng vừa vặn để khớp với lề tiêu đề
\noindent\hspace*{-6pt}%0
...
Mô hình vật lý định nghĩa cách thức dữ liệu được lưu trữ thực tế trên hệ quản trị cơ sở dữ liệu (sơ đồ ở Hình \ref{fig:physical_model}). Ở bước này, các kiểu dữ liệu (Data Types) cụ thể (INT, FLOAT, VARCHAR, DATE) và ràng buộc khóa ngoại (Foreign Keys) được thiết lập để tối ưu hóa không gian lưu trữ và tốc độ truy vấn. Chi tiết cấu trúc vật lý của các bảng chiều được mô tả trong Bảng \ref{tab:dim_tables_spec}. Các bảng sự kiện sản lượng và thời tiết được mô tả tương ứng trong Bảng \ref{tab:fact_gen_spec} và Bảng \ref{tab:fact_weather_spec}.
...
\begin{table}[H]
\centering
\caption{Cấu trúc vật lý của các bảng chiều (Dimension Tables)}
\label{tab:dim_tables_spec}
\begin{tabularx}{\textwidth}{p{3.5cm} p{3.2cm} c X}
\toprule
\textbf{Tên bảng} & \textbf{Tên trường} & \textbf{Kiểu DL} & \textbf{Diễn giải vật lý} \\
\midrule
\multirow{4}{*}{\texttt{dim\_solar\_site}} 
& site\_id & INT (PK) & Khóa chính của trạm. \\
& campus\_name & VARCHAR & Tên khu vực đặt trạm. \\
& capacity\_kw & FLOAT & Công suất thiết kế (kW). \\
& Inverter & VARCHAR & Thông tin bộ biến đổi điện. \\
\midrule
\multirow{3}{*}{\texttt{dim\_geography}} 
& geo\_id & INT (PK) & Khóa chính định danh địa lý. \\
& latitude & FLOAT & Vĩ độ (số thực). \\
& longitude & FLOAT & Kinh độ (số thực). \\
\midrule
\multirow{3}{*}{\texttt{dim\_date}} 
& date\_id & INT (PK) & Khóa chính đại diện ngày. \\
& full\_date & DATE & Định dạng thời gian (YYYY-MM-DD). \\
& day / month / year & INT & Ngày, tháng, năm phân tích. \\
\bottomrule
\end{tabularx}
\end{table}
...
\toprule
\textbf{Tên trường} & \textbf{Kiểu DL} & \textbf{Loại khóa} & \textbf{Diễn giải ý nghĩa} \\
\midrule
gen\_id & INT & PK & Định danh dòng sự kiện (Auto-increment). \\
site\_id & INT & FK & Tham chiếu \texttt{dim\_solar\_site}. \\
geo\_id & INT & FK & Tham chiếu \texttt{dim\_geography}. \\
date\_id & INT & FK & Tham chiếu \texttt{dim\_date}. \\
time\_id & INT & FK & Tham chiếu \texttt{dim\_time}. \\
energy\_generated\_kwh & FLOAT & Measure & Sản lượng điện năng sinh ra. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Tên trường} & \textbf{Kiểu DL} & \textbf{Loại khóa} & \textbf{Diễn giải ý nghĩa} \\
\midrule
weather\_id & INT & PK & Mã định danh lịch sử thời tiết. \\
geo\_id & INT & FK & Tham chiếu \texttt{dim\_geography}. \\
date\_id & INT & FK & Tham chiếu \texttt{dim\_date}. \\
time\_id & INT & FK & Tham chiếu \texttt{dim\_time}. \\
weather\_type\_id & INT & FK & Tham chiếu \texttt{dim\_weather\_type}. \\
is\_day & INT & Attribute & Trạng thái ánh sáng (1: Ngày, 0: Đêm). \\
shortwave\_radiation & INT & Measure & Bức xạ sóng ngắn. \\
temperature\_c & FLOAT & Measure & Nhiệt độ không khí. \\
cloud\_cover\_total & FLOAT & Measure & Tổng độ che phủ mây (\%). \\
\bottomrule
\end{tabularx}
\end{table}
