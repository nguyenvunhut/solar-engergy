...
Tổng hợp kết quả sau quy trình làm sạch và điền khuyết dữ liệu chuỗi thời gian:
\begin{itemize}
    \item Tổng số dòng toàn bộ kho dữ liệu: $2.731.946$ dòng ($100{,}0\%$).
    \item Số dòng dữ liệu đo đạc gốc ban ngày (\texttt{original}): $1.195.645$ dòng ($43{,}77\%$).
    \item Tổng số ô khuyết thiếu được điền sạch: $1.536.301$ dòng ($56{,}23\%$), trong đó:
    \begin{itemize}
        \item Điền qua quy tắc ban đêm \& ngưỡng bức xạ khởi động Inverter (\texttt{rule\_based\_night}): $1.383.493$ dòng ($50{,}64\%$).
        \item Điền qua nội suy tuyến tính thời gian ngắn $\le 2$ dòng (\texttt{linear}): $53.684$ dòng ($1{,}96\%$).
        \item Điền qua nội suy bảo toàn đơn điệu PCHIP Spline $3$--$8$ dòng (\texttt{cubic}): $50.704$ dòng ($1{,}86\%$).
        \item Điền qua mô hình hồi quy đa biến diện rộng $> 8$ dòng (\texttt{regression}): $48.420$ dòng ($1{,}77\%$).
    \end{itemize}
    \item Số ô NULL còn lại trong kho dữ liệu: $0$ ô (Điền khuyết thành công $100{,}0\%$).
\end{itemize}
