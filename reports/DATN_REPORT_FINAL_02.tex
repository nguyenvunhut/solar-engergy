...
Để giải quyết triệt để bài toán này, nhóm nghiên cứu phát triển và triển khai chiến lược **Điền khuyết Nhân quả Đa tầng (Causal Cascade Imputation Strategy)** kết hợp cơ chế kẹp trần vật lý ($1{,}20\times P_{\text{stc}}$) trong mã nguồn \texttt{srcs/02\_transform/02\_run\_hybrid\_imputation.py}.
...
Một đóng góp quan trọng trong quy trình tiền xử lý là việc tích hợp cờ nhị phân thiên văn học \texttt{is\_day} ($0$: Ban đêm, $1$: Ban ngày) từ mô hình khí tượng tái phân tích ERA5-Land (Open-Meteo API)~\cite{openmeteo2025isday}. Khác với các biến thời tiết viễn thám thông thường được nội suy từ lưới dự báo khí tượng, biến \texttt{is\_day} được tính toán trực tiếp thông qua **Thuật toán Vị trí Nhật tâm / Địa tâm (Astronomical Solar Position Algorithm)** dựa trên tọa độ thực tế của từng trạm phát:
...
\begin{enumerate}[label=\arabic*., leftmargin=*]
    ...
    \item \textbf{Phương trình thời gian ($EoT$) và Độ xích vĩ Mặt Trời ($\delta$):}
    \begin{align}
    ...
    \item \textbf{Giờ Mặt Trời thực ($TST$) và Góc giờ ($\omega$):} Hiệu chỉnh độ lệch kinh độ $\lambda$ và múi giờ $TZ$:
    \begin{equation}
    \Delta t = EoT + 4\lambda - 60 \cdot TZ \implies TST = (Hour_{\text{local}} \times 60 + Minute) + \Delta t
    \end{equation}
    \begin{equation}
    \omega = \left( \frac{TST}{4} \right) - 180^\circ
    \end{equation}

    \item \textbf{Góc nâng Mặt Trời ($\alpha$, Solar Elevation Angle) và Quyết định Nhị phân:}
    \begin{equation}
...
Dưới đây là trích đoạn mã nguồn thực thi từ \texttt{srcs/02\_transform/02\_run\_hybrid\_imputation.py}:
...
\begin{itemize}
    ...
    \item \textbf{Tổng số lượng bản ghi gắn cờ ngoại lai:} Toàn bộ pipeline đã gắn cờ thành công $\mathbf{7.431}$ dòng trên tổng số $\mathbf{2.731.946}$ dòng dữ liệu, tương ứng với tỷ lệ ngoại lai được kiểm soát ở mức lý tưởng $\mathbf{0{,}27\%}$.
\end{itemize}
...
Hai nhóm lý do chiếm phần lớn là đồng thuận GMM--IF ($6.102$ dòng) và bước nhảy phân phối cục bộ ($1.726$ dòng). Đặc biệt, nhờ dung sai trần vật lý được chuẩn hóa ở mức $1{,}20\times P_{\text{stc}}$, toàn bộ các đỉnh công suất quang điện thực tế do hiện tượng Cloud Enhancement ($100\% - 120\%$) được bảo toàn trọn vẹn là điểm dữ liệu bình thường, đưa số cờ vi phạm quá công suất danh định \texttt{PHYSICAL\_OVER\_CAPACITY} về đúng $0$ dòng ngoại lai sai lệch.
...
\midrule
\texttt{PHYSICAL\_OVER\_CAPACITY} ($> 1{,}20\times$) & $0$      & $0$      & $0$      & $\mathbf{100{,}0\%}$ \\
\texttt{PHYSICAL\_HIGH\_ENERGY\_NO\_SUN}   & $33$     & $33$     & $33$     & $\mathbf{100{,}0\%}$ \\
\texttt{PHYSICAL\_HIGH\_ENERGY\_LOW\_RAD}  & $131$    & $131$    & $131$    & $\mathbf{100{,}0\%}$ \\
\texttt{PHYSICAL\_LOW\_ENERGY\_STRONG\_SUN}& $56$     & $56$     & $56$     & $\mathbf{100{,}0\%}$ \\
\texttt{PHYSICAL\_DISTRIBUTION\_JUMP}      & $1.726$  & $1.726$  & $1.726$  & $\mathbf{100{,}0\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
Chỉ $1.195.645$ dòng ($43{,}77\%$) là số đo thật ban ngày. Phần còn lại do tầng ETL xử lý: $1.536.301$
dòng điền khuyết ($56{,}23\%$), gồm $1.383.493$ dòng gán theo quy tắc ban đêm và bức xạ dưới ngưỡng khởi động Inverter, $53.684$ dòng nội suy tuyến tính, $50.704$ dòng nội suy PCHIP Spline và $48.420$ dòng hồi quy đa biến. Ngoài ra, tầng ETL gắn cờ ngoại lai chính xác cho $7.431$ dòng ($0{,}27\%$), gồm $6.102$ dòng do hai cơ chế thống kê GMM--IF cùng đồng ý ($5.556$ dòng thuần $GMM \wedge IF$ và $546$ dòng kết hợp luật vật lý) cùng $1.329$ dòng trúng các luật rào chắn vật lý độc lập (chủ yếu là bước nhảy phân phối cục bộ $1.211$ dòng và bất thường bức xạ $118$ dòng); Mục~\ref{subsec:solar_geometry} kiểm chứng lại các cờ này.
...
\begin{itemize}
    ...
    \item \textbf{Quy trình tiền xử lý và kỹ thuật đặc trưng chuyên sâu:} Xây dựng pipeline tự động hóa xử lý triệt để $1.536.301$ ô dữ liệu khuyết thiếu (đạt $100{,}0\%$) thông qua chiến lược nội suy lai kết hợp quy tắc vật lý ban đêm \& ngưỡng bức xạ khởi động Inverter và nội suy phân cấp (Linear, PCHIP Spline, Multivariate Regression) có kẹp trần công suất vật lý $1{,}20\times P_{\text{stc}}$. Đồng thời, dự án giải quyết thành công bài toán lệch pha tần suất ghi nhận (đồng bộ chu kỳ 15 phút lên 1 giờ) và trích xuất các đặc trưng thời gian kết hợp lịch trình vận hành học xá (\texttt{is\_semester}, \texttt{is\_exam}, \texttt{is\_holiday}) nhằm loại bỏ triệt để hiện tượng rò rỉ dữ liệu (Data Leakage).
    \item \textbf{Mô hình học máy phát hiện dị thường và dự báo sản lượng:} Triển khai thành công kiến trúc phân lớp lai 3 tầng GMM--IF (Gaussian Mixture Model kết hợp Isolation Forest) trên các phân đoạn cây quyết định, kết hợp bộ 6 mã rào chắn vật lý để gắn cờ chính xác $7.431$ bản ghi ngoại lai (tỷ lệ $0{,}27\%$) phục vụ chẩn đoán bảo trì. Đồng thời, xây dựng và tối ưu hóa mô hình học máy (LightGBM kết hợp Optuna) dự báo sản lượng điện theo giờ, tích hợp phân tích SHAP nhằm giải thích định lượng mức độ đóng góp của các yếu tố khí tượng và nhiệt độ.
