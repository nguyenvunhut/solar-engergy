...
Xây dựng quy trình học máy tuân thủ nguyên lý nhân quả nghiêm ngặt (chống rò rỉ dữ liệu), trích xuất 39 đặc trưng chọn lọc (mở rộng thành 52 đặc trưng sau khi thêm các biến trễ mục tiêu $T+h$ cho mô hình dự báo trực tiếp H1) từ chuỗi thời gian, bức xạ quang học và góc chiếu mặt trời tất định Haurwitz/NOAA:
\begin{itemize}[noitemsep]
    \item Đánh giá trên sơ đồ kiểm tra chéo chuỗi thời gian 3-Fold Expanding Window; tối ưu hóa siêu tham số mô hình LightGBM hồi quy L1/Huber bằng thuật toán Bayesian TPE qua 20 Trials Optuna.
    \item Mô hình đạt sai số phần trăm tuyệt đối gia quyền WAPE $17{,}46\%$ ở tầm ngắn H1 ($T + 15$ phút) và $21{,}60\%$ ở tầm trung H4 ($T + 60$ phút) trên tập kiểm thử (Test) niêm phong đo thật ban ngày, cải thiện vượt trội so với mô hình cơ sở Facebook Prophet (Skill Score đạt $+50{,}09\%$ ở H1 và $+37{,}80\%$ ở H4).
...
Siêu dữ liệu dạng chữ được điền theo giá trị liền kề của chính trạm đối với
\texttt{panel} và \texttt{inverter}, và theo hằng số quy ước đối với \texttt{optimizers}
($1.259.007$ dòng, tức $46{,}08\%$) cùng \texttt{site\_metric}. Biến khí tượng ở đợt trích
...
\texttt{PHYSICAL\_HIGH\_ENERGY\_LOW\_RAD}  & $131$    & $144$    & $125$    & $\mathbf{95{,}4\%}$ \\
...
Ngưỡng là $\pm 5$ phút và không trạm nào được vượt. Đo trên $289.268$ điểm ban ngày, độ
...
trình bày ngay dưới đây. Bản hiện hành thay bằng hàm
\texttt{pvlib.solarposition.get\_solarposition} của thư viện \texttt{pvlib}
phiên bản $0{,}15{,}2$~\cite{holmgren2018pvlib}, tức thuật toán SPA của Reda và
...
Bản trước dùng quy ước $11$ giờ cho tháng $10$--$3$ và $10$ giờ cho tháng $4$--$9$; xem
đoạn đối chiếu bên dưới về giá phải trả của cách xấp xỉ đó \\
\addlinespace
Góc phương vị &
...
\textbf{Vì sao phải bỏ cách cắt mùa theo nguyên tháng.} Giờ mùa hè ở Úc bắt đầu Chủ nhật
đầu tiên của tháng $10$ và kết thúc Chủ nhật đầu tiên của tháng $4$, không rơi đúng vào
ranh giới tháng. Quy ước cắt theo nguyên tháng vì thế gán sai một giờ cho những ngày nằm
giữa đầu tháng và Chủ nhật đầu tiên, khoảng $12$ ngày mỗi năm. Sai một giờ trong góc giờ
kéo theo sai vài độ trong góc cao, mà góc cao lại nằm ở mẫu số của phép chuẩn hoá mục
tiêu, nên sai số đó truyền thẳng vào nhãn huấn luyện.

Notebook \texttt{03\_2b\_doi\_chieu\_pvlib.ipynb} đo cụ thể cái giá ấy trên $84.000$ điểm
của $42$ trạm, so từng điểm với \texttt{pvlib} làm chuẩn. Kết quả tóm tắt ở
Bảng~\ref{tab:doi_chieu_pvlib}: giữ quy ước tháng thì góc cao lệch tới $8{,}5403^\circ$ ở
phân vị $99$ và $12{,}3542^\circ$ ở mức lớn nhất; chuyển sang lịch múi giờ thật thì sai số
về đúng $0{,}0000^\circ$ ở cả hai mức. Công thức Haurwitz do nhóm tự cài cũng khớp
\texttt{pvlib.clearsky.haurwitz} tới $0{,}0000$ W/m$^2$.

\begin{table}[htbp]
\centering
\caption{Đối chiếu hình học Mặt Trời với \texttt{pvlib} trên $38.902$ điểm ban ngày}
\label{tab:doi_chieu_pvlib}
\small
\begin{tabular}{@{}l r r r r@{}}
\toprule
\textbf{Cách quy đổi múi giờ} & \textbf{Góc cao p99} & \textbf{Góc cao max} &
\textbf{Phương vị p99} & \textbf{Phương vị max} \\
\midrule
Quy ước theo nguyên tháng (bản trước) & $8{,}5403^\circ$ & $12{,}3542^\circ$ &
$11{,}6713^\circ$ & $26{,}6711^\circ$ \\
\textbf{Lịch giờ mùa hè thật (hiện hành)} & $\mathbf{0{,}0000^\circ}$ &
$\mathbf{0{,}0000^\circ}$ & $\mathbf{0{,}0000^\circ}$ & $\mathbf{0{,}0000^\circ}$ \\
\bottomrule
\end{tabular}
...
Dòng thứ hai bằng $0$ ở mọi cột nghĩa là phần hình học Mặt Trời của báo cáo này tái lập
đúng \texttt{pvlib}, không phải một bản cài đặt riêng chạy song song.
...
Giá trị đổi rõ giữa các fold, trung vị từ $1{,}450$ lên $1{,}559$, nên việc dùng chung
...
\footnotesize\renewcommand{\baselinestretch}{1.0}\selectfont
\begin{verbatim}
   v5_train_spatial.parquet: ty le buoc thay doi buc xa (chi ban ngay)
   26.7% -> 76.7%  [750,564/1,550,856 dong ban ngay]
\end{verbatim}
\par\vspace{8pt}
\endgroup
...
\textbf{Cách xử lý.} Quy trình khai báo một danh sách $21$ cột tất định hình học
...
tầm H1 sinh được $13$ cột, nâng tổng số đặc trưng vào mô hình từ $39$ lên $52$.
...
lên $6{,}5246$, tức đuôi phân bố có những điểm gấp gần năm lần phần thân. Mức $0{,}99$ là
điểm mà đường cong bắt đầu phẳng mà vẫn chặn được đuôi đó.

\textbf{Ngưỡng trong bảng và trong cấu hình là một.} Phép quét ở Bảng~\ref{tab:quet_phan_vi}
...
\textbf{Ngưỡng cắt của hai tầm và cấu tạo của mẫu số.} Bản dựng đầu tiên lấy mẫu số tại
...
\textbf{Cách xử lý.} Quy trình chuyển mẫu số sang chính thời điểm cần dự báo, tức lấy
...
\textbf{Kết quả.} Hai cách dựng mẫu số được so trực tiếp trên cùng dữ liệu, cùng bộ siêu tham
...
cùng bằng $1{,}3686$, nên mỗi tầm tự suy ngưỡng của mình mà không phải mượn ngưỡng của tầm kia.

\textbf{Phạm vi đánh giá đi kèm.} Vì mẫu số lấy tại $T+h$, điều kiện ban ngày cũng xét tại
...
trần, và chính sách trọng số bằng thực nghiệm bù trừ. Mỗi phép quét giữ nguyên mọi thứ còn
lại, rồi huấn luyện và chấm lại trên tập kiểm định.
...
\textbf{Cận cắt là tham số có tác động rõ, và không còn là hằng số đặt tay.} Nó được suy từ
...
$6{,}5246$, tức phép cắt gần như mất tác dụng chặn các điểm đo bất thường. Giá trị dùng thật
là $1{,}3686$ tại mức phân vị $0{,}99$.

\textbf{Hệ số nới trần được kiểm bằng đo trực tiếp phân bố.} Trên $636.138$ dòng đo thật ban
...
\textbf{Chính sách trọng số mẫu được chốt bằng thực nghiệm bù trừ.} Cơ chế trọng số vùng đỉnh
...
dữ liệu và cùng cấu hình: giữ $w = 1 + k$ cho WAPE $22{,}7428$ với thiên lệch trung bình
$+0{,}4532$ kWh; tắt trọng số cho $22{,}2789$ và $+0{,}1696$; trọng số tất định $w =$ mẫu số
chuẩn hoá cho kết quả tốt nhất $22{,}1273$ và $+0{,}1070$, đồng thời thẳng hàng với tử số
...
\textbf{Cột trùng lặp.} Ở ngưỡng $|r| \ge 0{,}95$ notebook đếm được $43$ cặp, nhưng chỉ những
...
Bốn đặc trưng được bổ sung nằm ở hạng $39$, $40$, $49$ và $56$ theo điểm thông tin tương
...
Ba mươi chín đặc trưng cơ bản đã chọn (mở rộng thành $52$ đặc trưng khi thêm các biến trễ mục tiêu $T+h$ cho mô hình H1) được áp đồng loạt cho mười tập dữ liệu bốn tập chính và sáu phần
của ba fold:
...
\begin{table}[htbp]
\centering
\caption{Danh mục 39 đặc trưng cơ bản được lựa chọn vào mô hình}
...
\textbf{Chỗ chạm biên miền tìm kiếm.} Năm siêu tham số (\textit{hyperparameter}) dừng sát
...
Các điều kiện theo \textbf{dòng} chỉ được áp sau khi nhãn đã dựng xong. Kết quả: $483.214$ dòng
ở tầm H$1$ và $483.094$ dòng ở tầm H$4$. Kiểm lại trên tệp
\texttt{prediction\_audit.parquet}, mọi dòng đều có mốc nhãn cách mốc gốc đúng $15h$ phút ở cả
hai tầm.
...
$26{,}36\%$, trung vị $18{,}33\%$; ở H4 khoảng này là $16{,}29\%$ đến $33{,}33\%$, trung vị
$23{,}22\%$. Chênh lệch hơn hai lần giữa hai đầu cho thấy một mô hình dùng chung cho cả $40$
...
Chênh lệch $17{,}63$ điểm WAPE ở H1 là kết quả cuối cùng của hai phương án, nhưng
...
lọt nhóm mười đầu nhưng vẫn được mô hình dùng thật: \texttt{ghi\_cs\_mt} đứng hạng $12$,
...
từ \texttt{site\_id\_enc} ($+0{,}12$) cùng \texttt{diffuse\_ratio} ($+0{,}08$) và
\texttt{direct\_normal\_irradiance} ($+0{,}08$, ứng với trực xạ $443$ W/m\textsuperscript{2}) ---
trời quang giữa trưa, đúng nhánh lên của đường sản lượng trong ngày. Ở ca kéo xuống, hai đặc
trưng đẩy mạnh nhất theo chiều âm là \texttt{diffuse\_ratio} ($-0{,}11$) với giá trị bằng $1$ ---
toàn bộ bức xạ là khuếch tán, tức trời phủ mây kín --- và \texttt{direct\_normal\_irradiance}
($-0{,}09$) bằng $0$, kèm \texttt{rolling\_max\_4} ($-0{,}09$): chiều muộn ngày nhiều mây, cả
lịch sử một giờ gần nhất cũng yếu. Ở ca gần nền, tổng đóng góp gần triệt tiêu vì các phần dương
và âm cân nhau. Hai hình dạng thác tương
...
$17{,}46\%$ ở tầm $15$ phút và $21{,}60\%$ ở tầm một giờ trên tập test niêm phong, phạm vi đo
thật ban ngày. So với đối chứng Prophet trên cùng tập dòng, chỉ số vượt trội đạt $+50{,}09\%$
và $+37{,}80\%$.
...
\begin{itemize}
    ...
    \item \textbf{Sai số phân bố không đều giữa các trạm}, từ $12{,}69\%$ tới $26{,}36\%$ ở
    tầm H1, và $16{,}29\%$ tới $33{,}33\%$ ở tầm H4. Mô hình dùng chung chưa mô tả hết phần khác
    ...
    \item \textbf{Năm siêu tham số (\textit{hyperparameter}) dừng sát biên miền tìm kiếm}
    (\texttt{num\_leaves} của MAE--H1 bằng $64$ và của MSE--H4 bằng $252$ trên miền
    $[63;\ 255]$; \texttt{subsample} của Huber--H1 bằng $0{,}707$; \texttt{reg\_alpha} của
    MSE--H1 bằng $0{,}064$; \texttt{colsample\_bytree} của MSE--H4 bằng $0{,}998$), nên không
    thể tuyên bố cấu hình hiện tại là tối ưu.
