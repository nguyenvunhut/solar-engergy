...
Xây dựng quy trình học máy tuân thủ nguyên lý nhân quả nghiêm ngặt (chống rò rỉ dữ liệu), trích xuất 40 đặc trưng chọn lọc (mở rộng thành 53 đặc trưng sau khi thêm các biến trễ mục tiêu $T+h$ cho mô hình dự báo trực tiếp H1) từ chuỗi thời gian, bức xạ quang học và góc chiếu mặt trời tất định Haurwitz/NOAA:
\begin{itemize}[noitemsep]
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Phạm vi Không gian:} 42 trạm phát quang điện áp mái với tổng công suất danh định $2.428\,\text{kWp}$ phân bổ tại 5 cơ sở của Đại học La Trobe, Bang Victoria, Úc (Bundoora, Bendigo, Albury-Wodonga, Shepparton, và Mildura, trong đó Mildura đóng vai trò trạm đối chứng khí tượng) thuộc bộ dữ liệu mở UNISOLAR~\cite{wimalaratne2022unisolar}.
...
trúng nhiều luật và $713$ dòng trúng một luật vật lý khác; Mục~\ref{subsec:solar_geometry} kiểm chứng lại các cờ này.
...
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily, backgroundcolor=\color{gray!10}, frame=single, rulecolor=\color{gray!40}]
...
$52{,}30\%$ số quan sát bằng $0$, ban đêm và các khung giờ rìa ngày. Trung vị của
...
\textbf{Về các cột bị loại.} Hai cột \texttt{con\_cach\_tran} và \texttt{clearsky\_proxy} được sinh ra ở bước đặc trưng nhưng
...
Giá trị thu được và ghi vào \texttt{model\_config.json} là $1{,}3764$, tính trên
...
$8{,}59\%$, và phần bị cắt dồn vào sườn lên buổi sáng, xem
...
Bi Top K cat, da bo sung lai (4): ['azimuth_sin', 'temperature_c',
                                   'doy_sin', 'doy_cos']
Tong dac trung duoc chon: 36 (Top K) + 4 (bao ve) = 40
\end{lstlisting}
\vspace{2pt}\hrule\par\vspace{6pt}
\endgroup

Bốn đặc trưng được bổ sung nằm ở hạng $37$, $40$, $49$ và $54$ theo điểm thông tin tương
...
Bốn mươi đặc trưng cơ bản đã chọn (mở rộng thành $53$ đặc trưng khi thêm các biến trễ mục tiêu $T+h$ cho mô hình H1) được áp đồng loạt cho mười tập dữ liệu bốn tập chính và sáu phần
của ba fold:
...
Bảng~\ref{tab:bo_dac_trung} tổng hợp toàn bộ $40$ đặc trưng cơ bản được lựa chọn (mở rộng thành $53$ đặc trưng sau khi thêm các biến mục tiêu tương lai $T+h$ cho H1) đưa vào huấn luyện mô hình, phân loại theo 6 nhóm chức năng.

\begin{table}[htbp]
\centering
\caption{Danh mục 40 đặc trưng cơ bản được lựa chọn vào mô hình.}
...
bằng đúng $1{,}3764$, tức cận cắt suy từ phân vị $99$ ở Mục~\ref{subsec:chuan_hoa_muc_tieu} đang hoạt động.
\newpage
\FloatBarrier
...
nào lệch đúng $15h$ phút: H$1$ loại $3.303$ dòng còn $475.599$, H$4$ loại $12.964$
dòng còn $465.818$. Kiểm lại trên tệp \texttt{prediction\_audit.parquet} thì không còn
...
Điều đáng nói hơn con số là cách các con số đó được bảo vệ. Năm rào chắn ở Mục~\ref{subsec:nam_rao_chan} được kiểm
...
\newblock ``Solar Position Calculator,''
\newblock Earth System Research Laboratories, Global Monitoring Laboratory, [Online]. Available: \url{https://gml.noaa.gov/grad/solcalc/}.
