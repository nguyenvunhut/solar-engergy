...
\begin{itemize}[noitemsep]
    ...
    \item Mô hình đạt sai số phần trăm tuyệt đối gia quyền WAPE $17{,}74\%$ ở tầm ngắn H1 ($T + 15$ phút) và $22{,}62\%$ ở tầm trung H4 ($T + 60$ phút) trên tập kiểm thử (Test) niêm phong đo thật ban ngày, cải thiện vượt trội so với mô hình cơ sở Facebook Prophet (Skill Score đạt $+49{,}53\%$ ở H1 và $+36{,}26\%$ ở H4).
...
\textbf{Địa điểm khảo sát} & 5 khuôn viên (Victoria, Úc) & Bundoora, Bendigo, Albury-Wodonga, Shepparton, kèm Mildura làm cơ sở đối chứng khí tượng. \\
...
\begin{enumerate}[label=\arabic*., leftmargin=*]
    ...
    \begin{itemize}[noitemsep]
        \item \textbf{Anomaly Rate (\%):} $\text{Tỷ lệ dị thường} = \dfrac{\textnormal{Số bản ghi dị thường}}{\textnormal{Tổng số bản ghi}} \times 100\%$ (Gắn cờ chính xác $7.431$ dòng ngoại lai thực sự, chiếm $0{,}27\%$ sau cơ chế giao đồng thuận và kẹp trần vật lý $1{,}20\times$).
...
đặc trưng đã sinh xong, nên nó được tính ở notebook \texttt{04\_vif\_diagnostics} và trình bày ở Mục~\ref{subsec:chan_doan_dac_trung}.
...
\texttt{05e\_kiem\_chung/\allowbreak con\_so\_bao\_cao.json}). Ngưỡng thực sự nạp vào mô hình bắt buộc phải suy từ tập huấn
...
\textbf{Sàn mẫu số} $\varepsilon$ \textbf{không quyết định độ chính xác.} Toàn dải từ $0{,}00$ tới
...
đọc từ ô kết quả notebook \texttt{05b\_thuc\_nghiem\allowbreak \_tham\_so\_hardcode.ipynb} và cột
...
Notebook \texttt{05e\_kiem\_chung\allowbreak \_con\_so\_bao\_cao.ipynb} đo lại trên phạm vi khai báo tường minh:
...
\texttt{05e\_kiem\_chung/\allowbreak con\_so\_bao\_cao.json}.
...
trưng tất định tại $T{+}h$ ở Mục~\ref{subsec:khu_tre_mt} thực sự được mô hình dùng, chứ không phải cột thừa.
...
vụ này \textbf{đọc lại} \texttt{clip\_k} và các tham số chuẩn hoá từ \texttt{model\_config.json} của chính mô hình đang vận hành, đảm bảo tính nhất quán giữa pha huấn luyện và pha triển khai thực tế, thay vì khai báo lại trong mã phục vụ. Nhờ vậy phép nhân ngược từ $k$
...
\begin{figure}[htbp]
    ...
    \caption[Giao diện Trang 3: Dự báo và phân tích giả định]{Giao diện Trang 3: Dự báo và phân tích giả định (What-If Analysis)}
