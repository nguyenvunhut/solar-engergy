% =============================================================================
%  BÁO CÁO PHẦN HỌC MÁY — PIPELINE DỰ BÁO SẢN LƯỢNG QUANG ĐIỆN v4
%  Nhóm: The Outliers — Trường Cao đẳng FPT Polytechnic
%
%  Biên dịch: xelatex BaoCao_ML_v4.tex  (chạy 3 lần để mục lục và tham chiếu ổn định)
%
%  NGUYÊN TẮC BIÊN SOẠN:
%    - Chỉ trình bày phần học máy; phần ETL và kho dữ liệu thuộc báo cáo khác.
%    - Mọi con số trong tài liệu này đều trích từ tệp kết quả hoặc từ ô đã chạy
%      trong notebook tương ứng; tên notebook ghi ngay tại chỗ dùng số đó.
%    - Mọi hình đều là kết quả chạy thật, ghi rõ notebook sinh ra hình.
% =============================================================================
...
% Font đẳng khoảng có đủ dấu tiếng Việt, để dán nguyên văn ô kết quả notebook.
\setmonofont{JetBrainsMono Nerd Font Mono}[Scale=0.80]
\usepackage{geometry}
...
\usepackage{booktabs}
\usepackage{array}
% Cột kiểu p{...} nhưng canh trái thay vì canh đều hai bên. Ô bảng hẹp mà canh đều
% thì TeX phải giãn khoảng trắng rất rộng ở dòng cuối mỗi ô; canh trái tránh hẳn việc đó.
\newcolumntype{L}[1]{>{\raggedright\arraybackslash}p{#1}}
\usepackage{float}
\usepackage{caption}
\usepackage{xcolor}
\usepackage{microtype}
% ---- CẤU HÌNH TOÀN CỤC CHỐNG TRÀN LỀ --------------------------------------
% Cho phép ngắt dòng ở BẤT KỲ ký tự nào của đường dẫn/URL, nên tên tệp dài bao
% nhiêu cũng không tràn ra lề phải. \UrlOrds mở rộng tập ký tự được phép ngắt
% sang cả chữ cái thường, không chỉ dấu / và _ như mặc định.
...
% Cho TeX nới thêm khoảng trắng khi cần, thay vì để chữ chạy quá lề.
\setlength{\emergencystretch}{3em}
\hyphenpenalty=1000
\tolerance=2000

% \tep{...} — dùng cho MỌI đường dẫn tệp và tên notebook trong báo cáo.
% Tự ngắt dòng khi cần, không phải canh tay từng chỗ.
% Khai báo bằng \DeclareRobustCommand để dùng được cả trong \caption và \section
% (tham số di động), nơi lệnh thường sẽ vỡ. \detokenize xử lý dấu gạch dưới nên
% không phải viết \_ ở từng chỗ.
\DeclareRobustCommand{\tep}[1]{\texttt{\detokenize{#1}}}

% Việt hoá các nhãn mặc định của lớp article
...
% Nới giới hạn float vì tài liệu nhiều hình
...
% Trang chi chua float phai day it nhat 85% chieu cao, neu khong thi don float xuong
% chung voi van ban - tranh trang co hai hinh nam hai dau va mot khoang trong o giua.
\renewcommand{\floatpagefraction}{0.85}
\renewcommand{\dbltopfraction}{0.9}
\renewcommand{\dblfloatpagefraction}{0.85}
% flafter: khong cho float xuat hien TRUOC cho no duoc nhac toi trong van ban. Thieu
% goi nay thi hinh 15 co the bi day len truoc hinh 14.
\usepackage{flafter}

\title{\textbf{Thiết kế, Huấn luyện và Đánh giá\\Mô hình Dự báo Sản lượng Quang điện}}
\author{Nhóm The Outliers}
\date{}
...
Phần sản lượng bắt nguồn từ bộ dữ liệu mở \textbf{UNISOLAR} do Wimalaratne và cộng
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{05d_thuc_nghiem_rao_chan_vat_ly.ipynb}) Hai
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{05d_thuc_nghiem_rao_chan_vat_ly.ipynb}) Quét hệ số
...
\begin{tabular}{@{}p{4.1cm} p{4.6cm} p{4.7cm}@{}}
\toprule
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{02_EDA.ipynb}) \textbf{Trái:} histogram toàn bộ tập
...
\tep{rad_x_sinelev}: chúng cấp cho mô hình tín hiệu về mức che phủ \textbf{ngay tại bước hiện
tại}, thay vì bắt mô hình suy ra gián tiếp từ các mốc trễ.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{eda_mau_hinh_phat_dien.png}
    \caption{(Trích xuất từ Notebook \tep{02_EDA.ipynb}) Sản lượng theo bước $15$ phút tại
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{02_EDA.ipynb}) \textbf{Trái:} ACF quét $288$ bước
...
\tep{diffuse_solar_radiation} vọt từ $+0{,}38$ lên $+0{,}88$. Mức tăng đó \textbf{không phản
...
Cột \tep{cloud_cover_total} cho thấy rõ nhất: ban ngày nó mang dấu \textbf{âm} $-0{,}1591$
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{02_EDA.ipynb}) Ma trận tương quan Spearman trên
...
\tep{02_split_time_series} vì vậy cắt dữ liệu \textbf{theo thứ tự thời gian}, không
xáo trộn.
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{02_split_time_series.ipynb}) Cách chia dữ
...
\tep{fold_i_train_selected.parquet}, lấy mốc thời gian lớn nhất $T_{\max}$ của phần
...
một họ: \tep{03_1_features_time} sinh đặc trưng suy từ trục thời gian và từ lịch sử sản
lượng, \tep{03_2_features_spatial} sinh đặc trưng hình học Mặt Trời cùng các đại lượng chuẩn
hoá theo trạm, \tep{03_3_features_aggregate} sinh đặc trưng tương tác khí tượng và mã hoá
biến phân loại. Hai notebook sau đó thu hẹp lại: \tep{04_vif_diagnostics} chẩn đoán đa cộng
tuyến, \tep{05_select_features} chốt danh sách chính thức.
...
\begin{table}[htbp]
\centering
\caption{Ba nhóm đặc trưng sinh tại notebook \tep{03_1_features_time.ipynb}. Cột cuối ghi
...
ngưỡng. Kết quả in ra ở notebook \tep{06_1_train_mae}:
...
trong notebook \tep{06_1_train_mae.ipynb}.}
...
\begin{figure}[htbp]
    ...
    tính vị trí Mặt Trời mà hàm \tep{tinh_goc_mat_troi} của dự án dựa theo.}
    \label{fig:noaa}
\end{figure}
...
\begin{itemize}
    \item \tep{shortwave_radiation} --- GHI \textbf{đo được thực tế}, do Open-Meteo cung cấp.
...
\begin{table}[htbp]
\centering
\caption{Hệ số hiệu chỉnh trời quang \tep{cs_factor} theo từng fold. Đọc từ ô kết quả của
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{03_2_features_spatial.ipynb}) Bức xạ tại trạm $1$,
...
liệu và \textbf{chỉ từ phần huấn luyện}: \tep{site_scale} là phân vị $99$ của sản lượng ban
...
\label{tab:tuong_tac}
\small
\begin{tabular}{@{}L{4.4cm} L{4.6cm} r r@{}}
\toprule
...
\small
\begin{tabular}{@{}L{4.3cm} L{6.2cm} L{4.6cm}@{}}
\toprule
...
\textbf{không lọt vào bộ được chọn}: \tep{clearsky_proxy} tương quan $0{,}9973$ với
...
bản của chúng tại $T{+}h$ với hậu tố \tep{_mt}, đồng thời \textbf{giữ nguyên} bản tại $T$. Ở
...
trong tệp cấu hình. Notebook huấn luyện đặt \tep{CLIP_K = None} rồi gọi hàm
...
\begin{table}[htbp]
\centering
\caption{Số điểm bị ngưỡng \tep{CLIP_CSI} cắt. Đọc từ tệp kết quả
...
cột nào thực sự đi vào mô hình, với vai trò tách bạch: \tep{04_vif_diagnostics} \textbf{đo} và
xuất bằng chứng, \tep{05_select_features} \textbf{quyết định}. Notebook chẩn đoán không tự loại
...
Cột duy nhất mang nhãn khuyết cao là \tep{source_gap_id} với $97{,}68\%$ ô trống --- đúng bản
...
notebook \tep{04_vif_diagnostics.ipynb}.}
\label{tab:trung_lap}
\small
\begin{tabular}{l l r}
\toprule
...
khác nhau của quy trình rồi cùng tồn tại. Cặp \tep{diffuse_ratio} và \tep{dni_ratio} có
...
\midrule
\tep{direct_normal_irradiance} & $9999$   & Ba cột bức xạ ràng buộc nhau qua quan hệ hình học \\
\tep{diffuse_solar_radiation}  & $9999$   & \\
\tep{shortwave_radiation}      & $9999$   & \\
\tep{tran_cong_suat}           & $7.567{,}53$ & Cùng suy từ phân vị sản lượng của một trạm \\
\tep{site_scale}               & $7.516{,}67$ & \\
\tep{day_of_year}              & $5.228{,}18$ & Trùng thông tin với biến tháng \\
\tep{month}                    & $5.186{,}16$ & \\
\tep{sin_elevation}            & $2.846{,}97$ & Hàm xác định của cùng góc cao Mặt Trời \\
\bottomrule
\end{tabular}
\end{table}

Ba cột bức xạ đạt mức chặn trên vì $GHI$ bằng tổng của thành phần trực xạ chiếu xuống mặt ngang
và thành phần khuếch tán, nên biết hai cột là suy ra được cột còn lại. Cặp \tep{site_scale} và
\tep{tran_cong_suat} có $r = 0{,}9999$ vì cả hai là phân vị $99$ và phân vị $99{,}9$ của cùng
một chuỗi sản lượng.
...
\midrule
\tep{rolling_mean_4} & $2{,}265$ & $461{,}18$ \\
\tep{rolling_min_4}  & $2{,}251$ & $882{,}04$ \\
\tep{rolling_max_4}  & $2{,}224$ & $1.375{,}52$ \\
\tep{lag_4}          & $2{,}123$ & $27{,}77$ \\
\tep{ky_vong}        & $2{,}039$ & $21{,}18$ \\
\tep{pv_clr_lonij}   & $1{,}959$ & $10{,}80$ \\
\tep{lag_96}         & $1{,}883$ & $5{,}36$ \\
\bottomrule
\end{tabular}
\end{table}
...
Trước khi chấm điểm, notebook \tep{05_select_features} khai báo tường minh một danh sách cấm
...
\midrule
$1$  & \tep{rolling_max_4}   & $1{,}1724$ \\
$2$  & \tep{rolling_mean_4}  & $1{,}1251$ \\
$3$  & \tep{ky_vong}         & $1{,}0785$ \\
$4$  & \tep{rolling_min_4}   & $1{,}0582$ \\
$5$  & \tep{lag_96}          & $1{,}0018$ \\
$6$  & \tep{lag_4}           & $0{,}8315$ \\
$7$  & \tep{solar_elevation} & $0{,}8258$ \\
$8$  & \tep{sin_elevation}   & $0{,}7967$ \\
$9$  & \tep{ty_le_bao_hoa}   & $0{,}7949$ \\
$10$ & \tep{ghi_cs}          & $0{,}7939$ \\
\bottomrule
\end{tabular}
\end{table}
...
hỗ. Hai cột \tep{doy_sin} và \tep{doy_cos} mã hoá vị trí trong năm dưới dạng vòng; điểm của
...
\tep{selected_features.json} để các bước huấn luyện và phục vụ đọc lại đúng một danh sách duy
nhất.
...
Mười bốn cột \tep{_mt} sinh thêm ở bước huấn luyện đều thuộc hai nhóm đầu --- hình học Mặt Trời
...
\begin{table}[htbp]
\centering
\caption{Miền tìm kiếm siêu tham số khai báo cho Optuna. Đọc từ hàm \tep{tham_so_co_ban} trong
...
\begin{itemize}
    ...
    \item \textbf{Đo thật} --- thêm điều kiện \tep{nhan_energy_source = measured}.
    \item \textbf{Đo thật ban ngày} --- thêm điều kiện \tep{nhan_is_daylight = true}. Đây là
...
\textbf{sớm} của chuỗi, nơi lịch sử dùng để tính \tep{lag_96} và các cửa sổ trượt còn ngắn, nên
...
\tep{ghi_cs_mt}, tức bức xạ trời quang \textbf{tại thời điểm mục tiêu}.
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{08_explainable_ai.ipynb}) Biểu đồ beeswarm. Mỗi điểm
...
\begin{table}[htbp]
\centering
\caption{Ba dự báo được mổ xẻ. Đọc từ \tep{08_explain/local_shap_cases.csv}.}
...
\begin{figure}[htbp]
    ...
    \caption{(Trích xuất từ Notebook \tep{08_explainable_ai.ipynb}) Biểu đồ thác cho hai dự báo
...
\tep{srcs/07_dashboard/}. Ứng dụng đọc lại đúng các tệp kết quả mà Phần~$5$ dùng, không tính
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{streamlit_1_timeseries.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{streamlit_2_shap.png}
...
vụ này \textbf{đọc lại} \tep{clip_k} và các tham số chuẩn hoá từ \tep{model_config.json} của
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{streamlit_3_du_bao.png}
