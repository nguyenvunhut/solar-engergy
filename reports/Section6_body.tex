...
Phần này trình bày toàn bộ quy trình học máy của dự án, bắt đầu từ bản trích mà tầng ETL
bàn giao và kết thúc ở ứng dụng dự báo. Các bước trích xuất, biến đổi và
...
khuyết cho nhóm siêu dữ liệu và nhóm biến khí tượng; $22$ cột được điền, $4$ cột còn lại
giữ nguyên trạng thái khuyết vì lý do nêu ở cuối mục. Siêu dữ liệu trạm được điền theo
...
gắn nhãn ngoại lai nên khuyết là đúng bản chất --- cùng ba cột tham chiếu thời tiết
...
% Khoi ket xuat notebook 00b. Dong dai nhat 63 ky tu nen dung footnotesize thay vi
% small nhu khoi cua notebook 01 (khoi do dong dai nhat chi 41 ky tu).
\begingroup
...
cloud_cover_low                846973           0        846973
cloud_cover_mid                   218           0           218
cloud_cover_high                  218           0           218
wind_speed                     217946           0        217946
precipitation_mm                  218           0           218
sunshine_duration                 218           0           218
weather_code                      218           0           218
weather_condition                 218           0           218
weather_description               218           0           218

--- SO SÁNH MEAN (CHỈ CÁC CỘT METRIC) ---
                          Mean_Before   Mean_After     Diff_%
shortwave_radiation        185.851745   185.836915   0.007980
direct_normal_irradiance   122.843856   122.834054   0.007980
diffuse_solar_radiation     63.007889    63.002861   0.007980
temperature_c               14.901137    14.900950   0.001254
cloud_cover_total           56.379385    56.374886   0.007980
cloud_cover_low             29.793525    22.639245  24.012869
cloud_cover_mid             24.697917    24.695946   0.007980
cloud_cover_high            29.660785    29.658419   0.007980
wind_speed                  11.937033    11.887307   0.416569
precipitation_mm             0.098342     0.098334   0.007980
sunshine_duration         1383.611801  1383.501393   0.007980
\end{verbatim}
\endgroup
...
\begin{figure}[htbp]
    ...
    \caption{Phân bố mật độ GMM và điểm cô lập Isolation Forest trên dữ liệu trạm 27 (nguồn: \texttt{05d\_thuc\_nghiem\_rao\_chan\_vat\_ly.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Số lượng mẫu vượt qua điều kiện kết hợp GMM và Isolation Forest (nguồn: \texttt{07\_local\_gmm\_if\_summary.json}).}
...
\begin{table}[htbp]
\centering
\caption{Đối chiếu số lượng cờ dị thường với tầng ETL (nguồn: \texttt{05d\_thuc\_nghiem\_rao\_chan\_vat\_ly.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Ảnh hưởng của hệ số nhân \texttt{safe\_IQR} đến dữ liệu bị lọc (nguồn: \texttt{quet\_he\_so\_safe\_iqr.csv}).}
...
\begin{figure}[htbp]
    ...
    \caption{Quét hệ số nhân $m$ của \texttt{safe\_IQR} qua sáu mức (nguồn: \texttt{05d\_thuc\_nghiem\_rao\_chan\_vat\_ly.ipynb}).}
...
\begin{figure}[!htbp]
    ...
    \caption{Vòng đời phát triển và vận hành mô hình học máy (MLOps). Nguồn: \textit{Data Version Control}~\cite{aivietnam2025dvc}.}
...
\midrule
Điền khuyết & \texttt{00\_fill\_null\_imputation} & \texttt{v4\_final\_cleaned.parquet} \\
...
\begin{table}[htbp]
\centering
\caption{Thống kê mô tả sản lượng theo ba phạm vi (nguồn: \texttt{02\_EDA.ipynb}).}
\label{tab:eda_mo_ta}
\small
...
\begin{figure}[htbp]
    ...
    \caption{Phân phối sản lượng trên tập huấn luyện theo các phạm vi (nguồn: \texttt{02\_EDA.ipynb}).}
...
\begin{figure}[htbp]
    ...
    \caption{Đặc tuyến sản lượng ngày trời quang và ngày nhiều mây tại trạm 27 (nguồn: \texttt{02\_EDA.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Hệ số tự tương quan (ACF) chuỗi sản lượng tại các mốc trễ (nguồn: \texttt{02\_EDA.ipynb}).}
\label{tab:eda_acf}
\small
\begin{tabular}{l r r}
\toprule
...
\begin{figure}[htbp]
    ...
    \caption{Biểu đồ tự tương quan ACF và tương quan riêng phần PACF (nguồn: \texttt{02\_EDA.ipynb}).}
    \label{fig:eda_acf}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Tương quan giữa các biến khí tượng với sản lượng điện (nguồn: \texttt{02\_EDA.ipynb}).}
...
\begin{figure}[htbp]
    ...
    \caption{Ma trận tương quan Spearman giữa các đặc trưng ban ngày (nguồn: \texttt{02\_EDA.ipynb}).}
    \label{fig:eda_corr}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Phân chia dữ liệu chuỗi thời gian theo cửa sổ mở rộng (nguồn: \texttt{02\_split\_time\_series.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Kiểm tra hiện tượng chồng lấn nhãn giữa các fold (nguồn: \texttt{05b\_thuc\_nghiem\_tham\_so\_hardcode.ipynb}).}
\label{tab:chong_lan}
\small
...
\begin{table}[htbp]
\centering
\caption{Các nhóm đặc trưng thời gian và độ trễ (nguồn: \texttt{03\_1\_features\_time.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Lựa chọn các mốc trễ chuỗi thời gian (nguồn: \texttt{03\_1\_features\_time.ipynb}).}
\label{tab:chon_lag}
\small
...
\begin{table}[htbp]
\centering
\caption{So sánh cấu hình mô hình nháp và mô hình chính thức (nguồn: \texttt{06\_1\_train\_mae.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Hệ số hiệu chỉnh trời quang \texttt{cs\_factor} theo từng fold (nguồn: \texttt{03\_2\_features\_spatial.ipynb}).}
\label{tab:cs_factor}
\small
\begin{tabular}{l r r r}
\toprule
...
\begin{figure}[htbp]
    ...
    \caption{Làm mịn và nội suy dữ liệu bức xạ bước 15 phút tại trạm 1 (nguồn: \texttt{03\_2\_features\_spatial.ipynb}).}
    \label{fig:downscale}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Quy mô công suất 8 trạm quang điện tiêu biểu (nguồn: \texttt{03\_2\_features\_spatial.ipynb}).}
\label{tab:quy_mo}
\small
...
\begin{table}[htbp]
\centering
\caption{Tương quan của các đặc trưng tương tác với mục tiêu (nguồn: \texttt{03\_3\_features\_aggregate.ipynb}).}
\label{tab:tuong_tac}
\small
...
\begin{table}[htbp]
\centering
\caption{So sánh đặc trưng tất định tại $T$ và $T{+}h$ (nguồn: \texttt{kiem\_chung\_bao\_cao\_v4.py}).}
...
\begin{table}[htbp]
\centering
\caption{Quét ngưỡng phân vị cận cắt trên tập kiểm định (nguồn: \texttt{quet\_muc\_phan\_vi\_nguong\_cat.csv}).}
...
\begin{table}[htbp]
\centering
\caption{Tỷ lệ nhãn bị cắt theo khung giờ trong ngày (nguồn: \texttt{kiem\_chung\_bao\_cao\_v4.py}).}
...
\begin{table}[htbp]
\centering
\caption{Thực nghiệm quét 4 tham số cố định trên tập kiểm định (nguồn: \texttt{thuc\_nghiem\_tham\_so\_hardcode.csv}).}
...
\begin{table}[htbp]
\centering
\caption{Số lượng điểm dữ liệu bị cắt bởi ngưỡng \texttt{CLIP\_CSI} (nguồn: \texttt{thuc\_nghiem\_clip\_csi.csv}).}
\label{tab:clip_csi}
\small
\begin{tabular}{r r r}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Phân bố chẩn đoán đa cộng tuyến trên 86 đặc trưng (nguồn: \texttt{04\_vif\_diagnostics.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Các cặp đặc trưng trùng lặp và cột đại diện giữ lại (nguồn: \texttt{04\_vif\_diagnostics.ipynb}).}
\label{tab:trung_lap}
...
\begin{table}[htbp]
\centering
\caption{Tám đặc trưng có hệ số phóng đại phương sai (VIF) cao nhất (nguồn: \texttt{04\_vif\_diagnostics.ipynb}).}
\label{tab:vif}
\small
...
\begin{table}[htbp]
\centering
\caption{Đặc trưng dẫn đầu theo độ quan trọng PLS và hệ số VIF tương ứng (nguồn: \texttt{04\_vif\_diagnostics.ipynb}).}
\label{tab:pls}
\small
\begin{tabular}{l r r}
\toprule
...
\begin{figure}[htbp]
    ...
    \caption{Danh sách nhóm biến loại trừ trong quy trình chọn đặc trưng.}
    \label{fig:deny_list}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Mười đặc trưng có điểm thông tin tương hỗ (MI) cao nhất (nguồn: \texttt{05\_select\_features.ipynb}).}
\label{tab:mi}
\small
\begin{tabular}{r l r}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Danh mục 39 đặc trưng được lựa chọn vào mô hình (nguồn: \texttt{selected\_features.json}).}
...
\begin{table}[htbp]
\centering
\caption{Không gian tìm kiếm siêu tham số cho Optuna (nguồn: \texttt{06\_1\_train\_mae.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Sai số trên tập kiểm định theo các hàm mất mát (nguồn: \texttt{metrics\_val.json}).}
\label{tab:chon_loss}
\small
...
\begin{table}[htbp]
\centering
\caption{Sai số WAPE trên tập test niêm phong theo ba phạm vi (nguồn: \texttt{metrics\_overall.json}).}
...
\begin{table}[htbp]
\centering
\caption{Đối chiếu sai số WAPE giữa các giai đoạn đánh giá (nguồn: \texttt{optuna\_vs\_current\_wape.csv}).}
...
\begin{table}[htbp]
\centering
\caption{So sánh hiệu năng giữa LightGBM và baseline Prophet (nguồn: \texttt{prophet\_test\_summary.json}).}
\label{tab:prophet}
\small
\begin{tabular}{l r r r r r}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Phân tích độ trễ pha và độ lệch đỉnh của dự báo H1 (nguồn: \texttt{lech\_theo\_vi\_tri\_trong\_gio\_h1.csv}).}
...
\begin{table}[htbp]
\centering
\caption{Top 10 đặc trưng quan trọng nhất theo giá trị SHAP trung bình (nguồn: \texttt{shap\_importance.csv}).}
...
\begin{figure}[htbp]
    ...
    \caption{Biểu đồ SHAP Beeswarm thể hiện tác động của các đặc trưng (nguồn: \texttt{08\_explainable\_ai.ipynb}).}
...
\begin{table}[htbp]
\centering
\caption{Phân tích 3 trường hợp dự báo bằng SHAP cục bộ (nguồn: \texttt{local\_shap\_cases.csv}).}
...
\begin{figure}[htbp]
    ...
    \caption{Biểu đồ SHAP Waterfall giải thích hai trường hợp dự báo tương phản (nguồn: \texttt{08\_explainable\_ai.ipynb}).}
