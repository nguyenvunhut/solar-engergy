...
gắn nhãn ngoại lai nên khuyết là đúng bản chất --- hai cột công suất
...
direct_normal_irradiance   122.842735   122.832932  0.007980
diffuse_solar_radiation     63.009010    63.003983  0.007980
temperature_c               14.778382    14.778149  0.001576
cloud_cover_total           56.379112    56.374614  0.007980
cloud_cover_low             27.241638    27.239464  0.007980
cloud_cover_mid             24.697638    24.695667  0.007980
cloud_cover_high            29.660557    29.658190  0.007980
wind_speed                  11.685672    11.685439  0.001991
precipitation_mm             0.098342     0.098334  0.007980
sunshine_duration         1383.611337  1383.500930  0.007980
\end{verbatim}
\endgroup
...
\midrule
Điền khuyết & \texttt{00\_fill\_null\_imputation} & \texttt{v5\_final\_cleaned.parquet} \\
...
$\mathbf{52{,}47\%}$ số quan sát bằng $0$, ban đêm và các khung giờ rìa ngày. Trung vị của
...
NOAA --- chuỗi Fourier cho độ xích vĩ, phương trình thời gian và góc giờ. Cách làm đó cho
kết quả đủ dùng nhưng vướng một điểm yếu ở khâu quy đổi múi giờ, trình bày ngay dưới đây.
Bản hiện hành thay bằng hàm \texttt{pvlib.solarposition.get\_solarposition} của thư viện
\texttt{pvlib} phiên bản $0{,}15{,}2$, tức thuật toán SPA của Reda và Andreas, đồng thời
lấy lịch giờ mùa hè thật của múi \texttt{Australia/Melbourne} thay cho quy ước cắt theo
nguyên tháng.
...
\textbf{Cách xử lý.} Quy trình khai báo một danh sách $21$ cột \textbf{tất định} --- hình học
...
Giá trị thu được và ghi vào \texttt{model\_config.json} là $\mathbf{1{,}3686}$, tính trên
...
tệp cấu hình mô hình. Giá trị dùng để huấn luyện và để chấm điểm ở Mục~$7.5$ là $1{,}3686$.
...
\begin{table}[htbp]
\centering
\caption{Thực nghiệm quét sàn mẫu số $\varepsilon$ trên tập kiểm định (nguồn: \texttt{thuc\_nghiem\_tham\_so\_hardcode.csv}).}
...
đọc từ ô kết quả notebook \texttt{05b\_thuc\_nghiem\_tham\_so.ipynb} và cột
...
quả \texttt{05\_selected/quet\_eps\_mau\_chuan\_hoa.csv}.
...
\texttt{05b\_thuc\_nghiem\_tham\_so/tn11\_ablation\_trong\_so.csv}) so ba phương án trên cùng
...
\begin{table}[htbp]
\centering
\caption{Phân bố chẩn đoán đa cộng tuyến trên 85 đặc trưng (nguồn: \texttt{04\_vif\_diagnostics.ipynb}).}
...
thứ năm là giá trị cùng khung giờ ngày hôm trước, đều suy từ lịch sử của chính chuỗi sản lượng. Bảng xếp hạng này khớp với kết quả tự tương quan ở Mục~$2.2$: chuỗi mang
...
\texttt{minute\_of\_day}, \texttt{hour}, \texttt{hour\_cos},
\texttt{doy\_sin}, \texttt{doy\_cos} \\
\addlinespace
...
\texttt{direct\_normal\_irradiance}, \texttt{temperature\_c} \\
\addlinespace
...
\texttt{ky\_vong}, \texttt{ty\_le\_bao\_hoa}, \texttt{tran\_cong\_suat}, \texttt{capacity\_kw},
\texttt{site\_id\_enc}, \texttt{inverter\_enc},
...
bằng đúng $1{,}3686$, tức cận cắt suy từ phân vị $99$ ở Mục~$7.3.5$ đang hoạt động.
...
Không phải mọi dòng trong tập test đều dùng để tính chỉ số, và thứ tự áp các điều kiện là điều
phải giữ đúng. Trước khi dựng nhãn, bước chấm điểm chỉ được lọc theo \textbf{trạm} --- bỏ hai
trạm có siêu dữ liệu công suất không đáng tin --- đưa $510.468$ dòng xuống $\mathbf{486.160}$
dòng. Lọc theo trạm an toàn vì mọi phép tra nhãn đều nằm trong từng trạm, nên bỏ cả một trạm
không làm thủng lưới thời gian của trạm khác.
...
Các điều kiện theo \textbf{dòng} chỉ được áp sau khi nhãn đã dựng xong. Kết quả:
$\mathbf{483.214}$ dòng ở tầm H$1$ và $\mathbf{483.094}$ dòng ở tầm H$4$. Kiểm lại trên tệp
...
\texttt{direct\_normal\_irradiance} --- một biến khí tượng đo được --- và vị trí thứ bảy thuộc về
nhóm mười đầu không có cột tất định tại $T{+}h$ nào.
...
thứ hạng. Thứ hai, nhóm đặc trưng tất định tại $T{+}h$ ở Mục~$3.4$ tuy không lọt nhóm mười đầu
nhưng vẫn được mô hình dùng thật: \texttt{ghi\_cs\_mt} đứng hạng $12$, \texttt{solar\_elevation\_mt}
hạng $15$, và bốn trong mười ba cột \texttt{\_mt} nằm trong nhóm hai mươi đầu, cộng lại chiếm
$6{,}2\%$ tổng giá trị SHAP. Chúng đóng vai trò bổ trợ chứ không thay thế các biến khí tượng đo được.
