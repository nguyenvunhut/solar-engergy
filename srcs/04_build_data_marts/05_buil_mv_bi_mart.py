...
def setup_materialized_views():
    ...
    WITH Raw_Joined AS (
    SELECT 
        gen.site_id, gen.geo_id, gen.date_id, gen.hourly_bucket, gen.total_energy AS e_hourly,
        -- Thêm các trường Weather
        w.shortwave_radiation, w.temperature_c, w.cloud_cover_total, 
        w.cloud_cover_low, w.cloud_cover_mid, w.cloud_cover_high,
        w.diffuse_solar_radiation, w.direct_normal_irradiance, w.wind_speed, w.precipitation_mm, w.sunshine_duration,
        -- Thêm các trường Calendar
        d.is_holiday, d.is_semester, d.is_exam, 
        CASE WHEN gen.hourly_bucket BETWEEN 6 AND 18 THEN true ELSE false END AS is_day,
        COALESCE(site.capacity_kw, 0) AS p_stc
    FROM (
        SELECT f.site_id, f.geo_id, f.date_id, t.hour AS hourly_bucket, SUM(f.energy_generated_kwh) AS total_energy
    ...
    -- ④ Output cuối: tất cả trường thô + tất cả measure
    SELECT
        -- === TRƯỜNG ĐỊNH DANH ===
    ...
    WITH daily_base AS (
        SELECT
            date_id, -- Đã giữ lại date_id thay cho report_date
            site_id,
            geo_id,
            is_holiday,
            is_semester,
            ...
            -- Sản lượng & doanh thu (SUM theo ngày)
            ...
            -- Thời tiết trung bình/tổng trong ngày
    ...
    kpi_calc AS (
        SELECT
            *,
            ...
            -- KPI 4: Time-Intelligence sản lượng (WTD, MTD, YTD)
            -- Lưu ý: DATE_TRUNC yêu cầu kiểu DATE/TIMESTAMP, nếu date_id là INT thì cần cast:
            SUM(e_daily) OVER (PARTITION BY site_id, DATE_TRUNC('week', to_date(date_id::text, 'YYYYMMDD')) ORDER BY date_id) AS wtd_energy,
            SUM(e_daily) OVER (PARTITION BY site_id, DATE_TRUNC('month', to_date(date_id::text, 'YYYYMMDD')) ORDER BY date_id) AS mtd_energy,
            SUM(e_daily) OVER (PARTITION BY site_id, DATE_TRUNC('year', to_date(date_id::text, 'YYYYMMDD')) ORDER BY date_id) AS ytd_energy,

            -- KPI 5: Time-Intelligence doanh thu (MTD, YTD)
            SUM(daily_revenue) OVER (PARTITION BY site_id, DATE_TRUNC('month', to_date(date_id::text, 'YYYYMMDD')) ORDER BY date_id) AS mtd_revenue,
            SUM(daily_revenue) OVER (PARTITION BY site_id, DATE_TRUNC('year', to_date(date_id::text, 'YYYYMMDD')) ORDER BY date_id) AS ytd_revenue
