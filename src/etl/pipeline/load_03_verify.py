...
if __name__ == "__main__":
    ...
    check(
        gen_db > 0,
        f"fact_solar_energy_gen loaded: {gen_db:,} rows (CSV has {len(gen_csv):,})",
    )
    ...
    check(
        wthr_db > 0,
        f"fact_weather loaded: {wthr_db:,} rows (CSV has {len(wthr_csv):,})",
    )
    ...
    cur.execute(
        "SELECT geo_id, location_name, capacity FROM dim_geography ORDER BY geo_id"
    )
    db_geo = cur.fetchall()
    for db_row in db_geo:
        ...
        if csv_row.empty:
            ...
            check(
                csv_row.iloc[0]["name"] == loc_name,
                f"geo_id={geo_id} name: CSV='{csv_row.iloc[0]['name']}' | DB='{loc_name}'",
            )
            cap_csv = csv_row.iloc[0]["capacity"]
            if pd.isna(cap_csv):
                check(
                    cap_db is None, f"geo_id={geo_id} capacity: CSV=NULL | DB={cap_db}"
                )
            else:
                check(
                    int(cap_csv) == int(cap_db),
                    f"geo_id={geo_id} capacity: CSV={cap_csv} | DB={cap_db}",
                )
    ...
    cur.execute(
        "SELECT site_id, capacity_kw FROM dim_solar_site ORDER BY site_id LIMIT 5"
    )
    for site_id, cap_db in cur.fetchall():
        ...
        if csv_row.empty:
            ...
            if pd.isna(cap_csv):
                check(
                    cap_db is None,
                    f"site_id={site_id} capacity_kw: CSV=NULL | DB={cap_db}",
                )
            else:
                check(
                    abs(float(cap_csv) - float(cap_db)) < 0.01 if cap_db else False,
    ...
    cur.execute(
        "SELECT date_id, full_date::text, is_holiday, is_semester, is_exam FROM dim_date ORDER BY date_id LIMIT 10"
    )
    for date_id, date_str, holiday_db, semester_db, exam_db in cur.fetchall():
        ...
        if csv_row.empty:
            ...
            check(
                h_csv == holiday_db,
                f"date={date_str} is_holiday: CSV={h_csv} | DB={holiday_db}",
            )
            check(
                s_csv == semester_db,
                f"date={date_str} is_semester: CSV={s_csv} | DB={semester_db}",
            )
            check(
                e_csv == exam_db, f"date={date_str} is_exam: CSV={e_csv} | DB={exam_db}"
            )
    ...
    cur.execute(
        "SELECT s.site_id, g.geo_id FROM dim_solar_site s JOIN dim_geography g ON s.campus_name = g.location_name"
    )
