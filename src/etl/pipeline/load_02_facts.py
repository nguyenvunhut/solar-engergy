...
def get_lookups(cur):
    ...
    cur.execute(
        "SELECT site_id, geo_id FROM dim_solar_site s JOIN dim_geography g ON s.campus_name = g.location_name"
    )
            ...
            for r in chunk.itertuples():
                ...
                rows.append(
                    (
                        ...
                        time_id,
                        None
                        if pd.isna(r.SolarGeneration)
                        else float(r.SolarGeneration),
                    )
                )
            execute_values(
                cur,
            ...
            for r in chunk.itertuples():
                ...
                rows.append(
                    (
                        ...
                        wt_id,
                        int(r.is_day),
                        float(r.shortwave_radiation),
                        float(r.temperature_2m),
                        float(r.cloud_cover),
                        float(r.cloud_cover_low),
                        float(r.cloud_cover_mid),
                        float(r.cloud_cover_high),
                        float(r.diffuse_radiation),
                        float(r.direct_radiation),
