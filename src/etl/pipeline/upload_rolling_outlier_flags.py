...
def run_verify_source(args: argparse.Namespace) -> int:
...
def run_upload(args: argparse.Namespace) -> int:
    ...
    if not args.execute:
        print()
        print("Dry-run only. Re-run with `upload --execute` to write Supabase.")
        return 0
    ...
    try:
        ...
        inserted = 0
        for batch in read_csv_batches(csv_path, args.batch_size):
        ...
        conn.commit()
        print("Upload committed.", flush=True)
        return 0
    except Exception:
        conn.rollback()
        raise
...
def run_apply_to_fact(_: argparse.Namespace) -> int:
    conn = connect_psycopg2()
    conn.autocommit = False
    cur = conn.cursor()
    try:
        ...
        if (fact_rows, fact_distinct, fact_dupes) != (2731946, 2731946, 0):
            raise RuntimeError("ABORT: fact key precheck failed")
        ...
        if (rows, true_flags, false_flags, null_flags) != (2731946, 100822, 2631124, 0):
            raise RuntimeError("ABORT: final count verification failed")
...
def run_inspect(_: argparse.Namespace) -> int:
    conn = connect_pg8000()
    try:
        conn.autocommit = True
        cur = conn.cursor()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    verify_parser = sub.add_parser(
        "verify-source",
        help="Read-only fingerprint check: Supabase fact table vs local flag CSV",
    )
    ...
    apply_parser = sub.add_parser(
        "apply-to-fact",
        help="Apply sparse rolling flags into staging.fact_solar_energy_gen by sitekey+timestamp",
    )
    apply_parser.set_defaults(func=run_apply_to_fact)
