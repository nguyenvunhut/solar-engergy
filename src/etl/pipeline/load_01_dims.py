...
WMO = {
    ...
    3: ("Overcast", "Full cloud cover"),
    45: ("Fog", "Depositing rime fog"),
    48: ("Rime fog", "Freezing fog"),
    51: ("Light drizzle", "Light intensity"),
    53: ("Moderate drizzle", "Moderate intensity"),
    55: ("Dense drizzle", "Dense intensity"),
    61: ("Slight rain", "Slight intensity"),
    63: ("Moderate rain", "Moderate intensity"),
    65: ("Heavy rain", "Heavy intensity"),
    71: ("Slight snow", "Slight intensity"),
    73: ("Moderate snow", "Moderate intensity"),
    75: ("Heavy snow", "Heavy intensity"),
    77: ("Snow grains", "Snow grains"),
    80: ("Slight showers", "Slight"),
    81: ("Moderate showers", "Moderate"),
    82: ("Violent showers", "Violent"),
    85: ("Slight snow showers", "Slight"),
    86: ("Heavy snow showers", "Heavy"),
    95: ("Thunderstorm", "Slight or moderate"),
    96: ("Thunderstorm+hail", "Slight hail"),
    99: ("Thunderstorm+hail", "Heavy hail"),
}
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 2. dim_solar_site
        df = sites.merge(
            campus[["id", "name"]], left_on="CampusKey", right_on="id", how="left"
        )
        ...
        execute_values(
            cur,
            "INSERT INTO dim_date (date_id,full_date,day,month,year,is_holiday,is_semester,is_exam) VALUES %s",
            rows,
        )
        ...
        execute_values(
            cur,
            "INSERT INTO dim_time (time_id,time_string,hour,minute) VALUES %s",
            rows,
        )
        ...
        combos = (
            ...
            .drop_duplicates()
            .sort_values(["weather_code", "is_day"])
        )
        rows = []
        for i, r in enumerate(combos.itertuples()):
            cond, desc = WMO.get(
                int(r.weather_code), ("Unknown", f"Code {r.weather_code}")
            )
            rows.append(
                (
                    i + 1,
                    int(r.weather_code),
                    int(r.is_day),
                    f"{cond} ({'Day' if r.is_day else 'Night'})",
                    desc,
                )
            )
        execute_values(
