# BÁO CÁO ĐỊNH LƯỢNG CHI TIẾT: HIỆU QUẢ CẢI TIẾN HIỆU SUẤT, THU HỒI NĂNG LƯỢNG VÀ TỐI ƯU HÓA KINH TẾ TỪ INSIGHTS DỰ ÁN
...
> **Vị trí tài liệu:** `docs/scrum_8_project_delivery_defense/2026_08_25_Bao_Cao_Dinh_Luong_Chi_Tiet_Insights_Va_De_Xuat_Cai_Tien.md`  
> **Cam kết khoa học:** $100\%$ số liệu phân rã 12 tháng (mùa hè vs mùa đông), công thức tính cắt ngọn biến tần (Inverter Clipping <sup>[[2]](#ref-2)</sup>), nguồn gốc MTTR (IEA-PVPS Task 13), phép tính góc nghiêng 12 tháng và ước tính lợi ích cơ chế tự rửa trôi được đối soát trực tiếp từ CSDL dự án và các tiêu chuẩn quốc tế.
...
2. [Cơ Sở Dữ Liệu Thực Tế Dự Án & Ma Trận Biểu Giá Thị Trường NEM <sup>[[8]](#ref-8)</sup> Từng Năm (2020–2022)](#2-cơ-sở-dữ-liệu-thực-tế-dự-án--ma-trận-biểu-giá-thị-trường-nem-từng-năm-20202022)
3. [Hạng Mục 1: Hệ Thống Pin Lưu Trữ Phân Tán BESS <sup>[[9]](#ref-9)</sup> Cấp Khuôn Viên & Phương Pháp Tính Cắt Ngọn Biến Tần 2,3%](#3-hạng-mục-1-hệ-thống-pin-lưu-trữ-phân-tán-bess-cấp-khuôn-viên--phương-pháp-tính-cắt-ngọn-biến-tần-23)
...
5. [Hạng Mục 3: Chuyển Đổi Sang Bảo Trì Dựa Trên Điều Kiện (CBM), Ánh Xạ 6 Mã Cờ Dị Thường GMM-IF <sup>[[6]](#ref-6)</sup> & Nguồn Gốc MTTR](#5-hạng-mục-3-chuyển-đổi-sang-bảo-trì-dựa-trên-điều-kiện-cbm-ánh-xạ-6-mã-cờ-dị-thường-gmm-if--nguồn-gốc-mttr)
...
* **Quy mô dữ liệu:** $2.731.946$ dòng chuỗi thời gian $15\,\text{phút}$ tại **42 trạm điện mặt trời áp mái** thuộc Đại học La Trobe (2020–2022).
* **Tổng công suất định danh DC:** $P_{\text{STC}} = 2.428\,\text{kWp}$ (phân bổ tại 5 khuôn viên: Bundoora $1.540\,\text{kWp}$, Bendigo $510\,\text{kWp}$, Albury-Wodonga $240\,\text{kWp}$, Shepparton $78\,\text{kWp}$, Mildura $60\,\text{kWp}$).
* **Sản lượng phát điện cơ sở trung bình:** $E_{\text{actual}} \approx 3.447.760\,\text{kWh/năm}$ (tương ứng Năng suất riêng $Yield \approx 1.420\,\text{kWh/kWp/năm}$).
...
* **Bản chất kỹ thuật:** Trong thiết kế 42 trạm áp mái tại La Trobe, tỷ lệ quá tải thiết kế $\text{ILR} = P_{\text{DC}} / P_{\text{AC}} \approx 1{,}20 - 1{,}30$. Vào những ngày nắng gắt mùa hè ($GHI \ge 900 - 1.050\,\text{W/m}^2$), công suất một chiều sinh ra từ giàn pin vượt quá công suất định mức xoay chiều cực đại của Inverter ($P_{\text{DC}}(t) \times \eta_{\text{inv}} > P_{\text{AC\_max}}$). Khi đó, biến tần tự động dịch chuyển điểm làm việc MPPT về phía $V_{\text{oc}}$ để xén bỏ phần công suất vượt trần.
...
* **Hiệu quả thu hồi bằng BESS DC-Coupled:** BESS hấp thụ trực tiếp dòng DC trước tầng nghịch lưu Inverter, thu hồi $88\%$ năng lượng sau hao hụt $\eta_{\text{RTE}}$:
...
### 3.2. Phương Pháp & Công Thức Tính Toán Công Suất, Dung Lượng & Khối Lượng Vật Lý BESS

Hệ thống pin lưu trữ BESS cho 42 trạm thuộc 5 khuôn viên độc lập được tính toán và định cỡ (sizing) dựa trên các nguyên lý kỹ thuật điện và cơ sở dữ liệu thực nghiệm:

#### 1. Tính toán Công suất định mức BESS ($P_{\text{BESS}}$ tính bằng $\text{kW}$ hoặc $\text{MW}$):
* **Hấp thụ công suất cắt ngọn cực đại trưa hè ($P_{\text{clip, peak}}$):**
  Với tỷ lệ quá tải thiết kế $\text{ILR} = 1{,}25$, ngưỡng trần biến tần AC là $P_{\text{AC, max}} = \frac{P_{\text{DC}}}{1{,}25} = 0{,}80 \times P_{\text{DC}}$.
  Công suất cắt ngọn cực đại tức thời trên toàn hệ thống:
  $$P_{\text{clip, peak}} = P_{\text{DC}} - P_{\text{AC, max}} = 0{,}20 \times P_{\text{DC}} = 0{,}20 \times 2.428\,\text{kWp} = \mathbf{485{,}6\,\text{kW}}$$
* **Mục tiêu gọt đỉnh phụ tải tiêu thụ giờ cao điểm (Peak Shaving):**
  Để giảm áp lực lưới và gọt bớt **$800\,\text{kW}$** công suất tiêu thụ đỉnh của khuôn viên trường, công suất định mức tổng của hệ thống biến tần chuyển đổi BESS (PCS) được chuẩn hóa là:
  $$P_{\text{BESS, total}} = \mathbf{1.000\,\text{kW} = 1{,}0\,\text{MW}} \quad (\approx 41{,}2\%\text{ tổng công suất } P_{\text{DC}})$$
* **Phân bổ công suất theo từng Campus:**
  $$P_{\text{BESS}, i} = P_{\text{BESS, total}} \times \left(\frac{P_{\text{DC}, i}}{P_{\text{DC, total}}}\right)$$

#### 2. Tính toán Dung lượng Năng lượng BESS ($E_{\text{BESS}}$ tính bằng $\text{kWh}$ hoặc $\text{MWh}$):
* **Thời lượng xả cao điểm ($\Delta t = 2{,}5\,\text{giờ}$, tương ứng tốc độ xả chuẩn $0{,}4\text{C}$):**
  $$E_{\text{BESS, total}} = P_{\text{BESS, total}} \times \Delta t_{\text{discharge}} = 1.000\,\text{kW} \times 2{,}5\,\text{h} = \mathbf{2.500\,\text{kWh} = 2{,}5\,\text{MWh}}$$
* **Năng lượng xả hữu ích hàng năm ($E_{\text{usable/year}}$):**
  Xét theo độ xả sâu an toàn $\text{DoD} = 90\%$ (đảm bảo tuổi thọ pin Lithium LFP trên 6.000–8.000 chu kỳ) và hiệu suất sạc-xả vòng lặp $\eta_{\text{RTE}} = 88\%$:
  $$E_{\text{usable/cycle}} = E_{\text{BESS}} \times \text{DoD} \times \eta_{\text{RTE}} = 2.500\,\text{kWh} \times 0{,}90 \times 0{,}88 = \mathbf{1.980\,\text{kWh/chu kỳ}}$$
  Với $\approx 360$ chu kỳ nạp/xả mỗi năm $\implies$ Tổng năng lượng điều tiết đạt **$712.182\,\text{kWh/năm}$** (trong đó thu hồi $69.782\,\text{kWh}$ điện cắt ngọn hữu ích).

#### 3. Tính toán Khối lượng & Trọng lượng Vật lý Phần Cứng ($M_{\text{BESS}}$ tính bằng Tấn):
* **Mật độ năng lượng trọng lượng cell pin Lithium Iron Phosphate (LiFePO4 / LFP công nghiệp):**
  $$w_{\text{density}} \approx 140\,\text{Wh/kg} = 0{,}14\,\text{kWh/kg}$$
* **Khối lượng module cell pin:**
  $$M_{\text{cells}} = \frac{E_{\text{BESS}}}{w_{\text{density}}} = \frac{2.500\,\text{kWh}}{0{,}14\,\text{kWh/kg}} \approx \mathbf{17{,}85\,\text{tấn}}$$
* **Tổng khối lượng hệ thống (Bao gồm vỏ tủ ngoài trời Outdoor Cabinet chống cháy IP65, hệ thống làm mát chất lỏng Liquid Cooling HVAC, tủ BMS và biến tần PCS):**
  $$M_{\text{total}} \approx M_{\text{cells}} \times 1{,}4 \approx \mathbf{25{,}0\,\text{tấn}}$$

---

### 3.3. Bảng Ma Trận Phân Bổ Cấu Hình BESS Chi Tiết Cho 5 Khuôn Viên (Campuses)

```
┌─────────────────┬──────────┬───────────┬──────────┬─────────────┬──────────────┬──────────────┬──────────────┬──────────────┬──────────────┐
│ Khuôn Viên      │ Số Trạm  │ Công Suất │ Tỷ Trọng │ Sản Lượng   │ Công Suất    │ Dung Lượng   │ Trọng Lượng  │ Năng Lượng Xả│ Gọt Đỉnh     │
│ (Campus Name)   │ (Sites)  │ DC (kWp)  │ (%)      │ Thực Tế     │ BESS (kW)    │ BESS (kWh)   │ Pin (Cells)  │ TOU + Clip   │ Demand (kW)  │
├─────────────────┼──────────┼───────────┼──────────┼─────────────┼──────────────┼──────────────┼──────────────┼──────────────┼──────────────┤
│ 1. Bundoora     │ 26 trạm  │ 1.540 kWp │ 63,4%    │ 2.187 MWh   │ 600 kW       │ 1.500 kWh    │ ~10,71 tấn   │ 451.713 kWh  │ 480 kW       │
│ 2. Bendigo      │ 8 trạm   │ 510 kWp   │ 21,0%    │ 725 MWh     │ 210 kW       │ 525 kWh      │ ~3,75 tấn    │ 149.593 kWh  │ 168 kW       │
│ 3. Albury-Wod   │ 4 trạm   │ 240 kWp   │ 9,9%     │ 341 MWh     │ 90 kW        │ 225 kWh      │ ~1,61 tấn    │ 70.397 kWh   │ 72 kW        │
│ 4. Shepparton   │ 2 trạm   │ 78 kWp    │ 3,2%     │ 111 MWh     │ 30 kW        │ 75 kWh       │ ~0,54 tấn    │ 22.879 kWh   │ 24 kW        │
│ 5. Mildura      │ 2 trạm   │ 60 kWp    │ 2,5%     │ 86 MWh      │ 20 kW        │ 50 kWh       │ ~0,36 tấn    │ 17.599 kWh   │ 16 kW        │
├─────────────────┼──────────┼───────────┼──────────┼─────────────┼──────────────┼──────────────┼──────────────┼──────────────┼──────────────┤
│ TỔNG CỘNG       │ 42 TRẠM  │ 2.428 kWp │ 100,0%   │ 3.450 MWh   │ 1.000 kW     │ 2.500 kWh    │ ~17,85 tấn   │ 712.182 kWh  │ 800 kW       │
│                 │          │           │          │             │ (1,0 MW)     │ (2,5 MWh)    │ (~25t tổng)  │              │              │
└─────────────────┴──────────┴───────────┴──────────┴─────────────┴──────────────┴──────────────┴──────────────┴──────────────┴──────────────┘
```
...
Trong thực tế dự án, tổn thất nhiệt độ phụ thuộc chặt chẽ vào bức xạ $GHI$ và nhiệt độ môi trường $T_{\text{amb}}$. Sự khác biệt giữa mùa hè và mùa đông được mô hình hóa theo phương trình thực nghiệm Sandia:
* **Mùa hè (Tháng 12, 1, 2):** $T_{\text{amb}} \approx 28 - 38^\circ\text{C}$, $GHI \ge 900\,\text{W/m}^2 \implies$ Khi lắp áp sát mái, $T_{\text{cell, flush}} = 68 - 72^\circ\text{C}$. Khe hở $150\,\text{mm}$ kích hoạt đối lưu mạnh hạ nhiệt $\Delta T_{\text{cell}} = -11^\circ\text{C} \text{ đến } -12^\circ\text{C}$, giúp cải thiện **$+4{,}1\% - +4{,}3\%$ sản lượng**.
* **Mùa đông (Tháng 6, 7, 8):** $T_{\text{amb}} \approx 4 - 12^\circ\text{C}$, $GHI \approx 200 - 450\,\text{W/m}^2 \implies T_{\text{cell, flush}} = 35 - 45^\circ\text{C}$. Khe hở chỉ hạ nhiệt $\Delta T_{\text{cell}} = -4^\circ\text{C} \text{ đến } -5^\circ\text{C}$, mức cải thiện đạt **$+1{,}56\% - +2{,}15\%$ sản lượng**.
...
* **Hiệu quả đầu tư:** Chi phí giá đỡ nhôm định hình $24.280\,\text{AUD} \implies$ **Thời gian hoàn vốn chính xác là $1{,}035\text{ năm}$ ($\approx 12{,}4\text{ tháng}$)**.
...
* **Quy trình vận hành truyền thống (Time-Based / Reactive O&M):**
  * Sự cố đứt cầu chì chuỗi pin ngầm hoặc Inverter ngắt quá á## 9. Hạng Mục 7: Chiến Lược Bảo Trì Làm Sạch Dựa Trên Lượng Mưa (Precipitation) & Chuỗi Ngày Khô Hạn

### 9.1. Cơ Sở Khoa Học Của Ngưỡng Mưa 5,0 mm/ngày (Ngưỡng Tự Làm Sạch Của Tự Nhiên)
Trong vận hành thực tế, mối quan hệ giữa lượng mưa và độ sạch quang học của tấm pin tuân theo các nghiên cứu thực nghiệm chuẩn ngành:
* **Ngưỡng mưa tự làm sạch ($\text{Rain} \ge 5{,}0\,\text{mm/ngày}$):**  
  Theo các công bố kinh điển của **Kimber et al. (NREL, 2006) <sup>[[10]](#ref-10)</sup>** và **Coello & Boyle (IEEE J-PV, 2019) <sup>[[11]](#ref-11)</sup>**, một trận mưa tích lũy từ $5{,}0\,\text{mm}$ trở lên mới đủ động năng tạo ra một lớp màng nước chảy xiết trên bề mặt kính tấm pin, cuốn trôi tới **$90\% - 95\%$** lượng bụi bẩn tích tụ. Do đó, sau mỗi ngày có lượng mưa $\ge 5{,}0\,\text{mm}$, biến đếm chuỗi ngày khô $DryStreak$ được tự động **reset về 0**.
* **Ngưỡng mưa bụi gây hại ($\text{Rain} < 5{,}0\,\text{mm/ngày}$):**  
  Lượng nước quá nhỏ không đủ tạo dòng chảy tràn mà chỉ làm ẩm lớp bụi trên mặt kính. Dưới bức xạ mặt trời, nước bốc hơi nhanh và làm lớp bụi khô bị đóng bánh (hiện tượng **"cementing / mud effect"**), làm tăng tổn thất quang học. Do đó, các ngày có mưa $< 5{,}0\,\text{mm}$ vẫn bị thuật toán tính là **ngày khô hạn tích lũy bụi**.

---

### 9.2. Phương Pháp & Công Thức Toán Học Tối Ưu Hóa Kinh Tế (Xác Định Chu Kỳ $N^* = 21\,\text{ngày}$)
Chu kỳ làm sạch tối ưu được dẫn xuất từ **Mô hình cân bằng chi phí O&M của NREL & IEA-PVPS Task 13**:

#### 1. Mô hình toán học tích lũy tổn thất do bám bụi:
Tổn thất công suất quang điện do bụi bẩn tăng tuyến tính theo số ngày khô $t$:
$$\text{Loss}_{\text{soiling}}(t) = k_{\text{soiling}} \times t$$
*(Trong đó: $k_{\text{soiling}} \approx 0{,}15\%/\text{ngày} = 0{,}0015/\text{ngày}$ là tốc độ bám bụi trung bình tại vùng ôn đới/nông nghiệp bang Victoria theo Mani & Pillai, 2010 <sup>[[12]](#ref-12)</sup>).*

Tổng thiệt hại doanh thu tiền điện tích lũy sau $N$ ngày khô hạn:
$$C_{\text{loss}}(N) = \sum_{t=1}^{N} \left( P_{\text{DC}} \times \text{PSH} \times k_{\text{soiling}} \times t \times \text{Tariff} \right) \approx \frac{1}{2} \cdot P_{\text{DC}} \cdot \text{PSH} \cdot k_{\text{soiling}} \cdot \text{Tariff} \cdot N^2$$

#### 2. Điểm cân bằng hòa vốn điều động nhân công ($N^*$):
Điểm tối ưu kinh tế $N^*$ để điều động đội rửa pin là thời điểm **chi phí tổn thất điện tích lũy bắt đầu vượt qua chi phí thuê nhân công rửa pin một lần ($C_{\text{clean}}$)**:
$$N^* = \sqrt{\frac{2 \times C_{\text{clean}}}{P_{\text{DC}} \times \text{PSH} \times k_{\text{soiling}} \times \text{Tariff}}}$$

#### 3. Thế số đối soát thực tế tại 42 trạm Đại học La Trobe:
* **Xét theo Campus lớn nhất (Bundoora $1.540\,\text{kWp}$):** Với $P_{\text{DC}} = 1.540\,\text{kWp}$, $\text{PSH} = 4{,}5\,\text{h/ngày}$, $\text{Tariff} = 0{,}20\,\text{AUD/kWh}$, $k_{\text{soiling}} = 0{,}0015/\text{ngày}$, chi phí rửa cụm Bundoora $C_{\text{clean}} \approx 750\,\text{AUD}$:
  $$N^* = \sqrt{\frac{2 \times 750}{1.540 \times 4{,}5 \times 0{,}0015 \times 0{,}20}} = \sqrt{\frac{1.500}{2{,}079}} = \sqrt{721{,}5} \approx \mathbf{26{,}86\,\text{ngày}}$$
* **Xét bình quân gia quyền toàn bộ 42 trạm (chi phí rửa từng cụm trạm $400 - 500\,\text{AUD}$):**
  $$N^* \approx \sqrt{\frac{2 \times 500}{3{,}28}} = \sqrt{304{,}8} \approx \mathbf{17{,}46\,\text{ngày}}$$
* $\implies$ Dải tối ưu kinh tế lý thuyết nằm trong khoảng **$18 - 27\,\text{ngày}$**. Nhóm chuẩn hóa điểm rơi điều độ vận hành là **$N^* = 21\,\text{ngày}$ (đúng 3 tuần liên tiếp)** để đồng bộ với lịch trực ca và bảo dưỡng của đội kỹ thuật trường.

---

### 9.3. Bảng Thống Kê Khí Tượng 12 Tháng Lượng Mưa & Tỷ Lệ Ngày Khô Tại Victoria

```
┌──────┬──────────┬──────────────────────────────┬─────────────────────────┬──────────────────────────────────────────┐
│Tháng │ Mùa Vụ   │ Lượng Mưa Trung Bình (mm/ng) │ Tỷ Lệ Ngày Khô Hạn (%)  │ Đánh Giá Nguy Cơ Tích Tụ Bụi Mùa Vụ      │
├──────┼──────────┼──────────────────────────────┼─────────────────────────┼──────────────────────────────────────────┤
│ T01  │ Mùa Hè   │ 3,96 mm/ngày                 │ 82,3%                   │ Tích tụ bụi nhanh, cần theo dõi chuỗi khô│
│ T02  │ Mùa Hè   │ 1,21 mm/ngày                 │ 93,4%                   │ Tích tụ bụi cao nhất năm (Khô hạn sâu)   │
│ T03  │ Mùa Thu  │ 1,91 mm/ngày                 │ 90,1%                   │ Tích tụ bụi nhanh, kích hoạt chuỗi 21 ng │
│ T04  │ Mùa Thu  │ 2,04 mm/ngày                 │ 89,1%                   │ Khô ráo, cần theo dõi API mưa            │
│ T05  │ Mùa Đông │ 2,03 mm/ngày                 │ 88,6%                   │ Khô ráo nhưng bức xạ thấp                │
│ T06  │ Mùa Đông │ 2,80 mm/ngày                 │ 80,4%                   │ Ẩm ướt, không cần rửa pin                │
│ T07  │ Mùa Đông │ 1,96 mm/ngày                 │ 85,4%                   │ Mưa phùn nhẹ, ít bụi                     │
│ T08  │ Mùa Đông │ 1,80 mm/ngày                 │ 87,9%                   │ Ẩm ướt, không cần can thiệp              │
│ T09  │ Mùa Xuân │ 2,89 mm/ngày                 │ 82,0%                   │ Mưa xuân tự rửa trôi                     │
│ T10  │ Mùa Xuân │ 3,38 mm/ngày                 │ 81,1%                   │ Mưa xuân tự rửa trôi                     │
│ T11  │ Mùa Hè   │ 2,50 mm/ngày                 │ 85,0%                   │ Bắt đầu vào mùa khô hạn                 │
│ T12  │ Mùa Hè   │ 1,60 mm/ngày                 │ 91,7%                   │ Tích tụ bụi rất nhanh                    │
├──────┼──────────┼──────────────────────────────┼─────────────────────────┼──────────────────────────────────────────┤
│ CẢ NĂM          │ 2,34 mm/ngày                 │ 86,4%                   │ Mùa hè và đầu thu cần kích hoạt CBM mưa  │
└─────────────────┴──────────────────────────────┴─────────────────────────┴──────────────────────────────────────────┘
```

---

### 9.4. Định Lượng Lợi Ích Vận Hành & Tài Chính
...
Bảng dưới đây tổng hợp chi tiết toàn bộ các tỷ lệ phần trăm cải thiện, sản lượng điện thu hồi (kWh/tháng, kWh/năm), giá trị tài chính theo từng năm thực tế (2020, 2021, 2022), mức bình quân 3 năm và thời gian hoàn vốn cho toàn bộ hệ thống $2.428\,\text{kWp}$ (42 trạm Đại học La Trobe):
...
│ 3  │ Bảo trì CBM & Anomaly       │ +2,04% tổng  │ 70.330 kWh   │ 26.659 AUD   │ 28.714 AUD   │ 32.528 AUD   │ 29.066 AUD   │ 8.000 AUD/năm│ < 4 Tháng    │
│ 4  │ Khung nghiêng chữ A 15° <sup>[[4]](#ref-4)</sup> mái │ +3,90% nhóm  │ 53.350 kWh   │ 9.603 AUD    │ 10.403 AUD   │ 12.537 AUD   │ 10.670 AUD   │ 18.000 AUD   │ 1,68 Năm     │
...
1. <a name="ref-1"></a>**[Thay chuẩn IEC 61724-1]** Deline, C., et al. (2024). *Irradiance monitoring for bifacial PV systems' performance and capacity testing*. IEEE Journal of Photovoltaics.
   ...
   * *(Nghiên cứu ứng dụng trực tiếp tiêu chuẩn IEC 61724-1 trong tính toán Performance Ratio <sup>[[1]](#ref-1)</sup>).*
...
10. <a name="ref-10"></a>**[Nghiên cứu Ngưỡng Tự Làm Sạch NREL]** Kimber, A., et al. (2006). *The Effect of Soiling on Large Grid-Connected Photovoltaic Systems in California and the Southwest Region*. IEEE 4th World Conference on Photovoltaic Energy Conversion.
    * [Mã DOI Quốc tế](https://doi.org/10.1109/WCPEC.2006.279690)
    * *(Nghiên cứu chuẩn xác lập ngưỡng mưa tự làm sạch $\ge 5{,}0\,\text{mm}$ giúp phục hồi $95\%$ hiệu suất).*

11. <a name="ref-11"></a>**[Nghiên cứu Nhận Diện Trạng Thái Làm Sạch]** Coello, M., & Boyle, L. (2019). *Estimating the Cleaned State of Photovoltaic Modules from Precipitation Data*. IEEE Journal of Photovoltaics, 9(5), 1380–1385.
    * [Mã DOI Quốc tế](https://doi.org/10.1109/JPHOTOV.2019.2924185)
    * *(Thuật toán nhận diện chuỗi ngày khô và mô hình hóa trạng thái phục hồi quang học của tấm pin từ chuỗi dữ liệu lượng mưa).*

12. <a name="ref-12"></a>**[Tổng Quan Tổn Thất Bám Bụi Toàn Cầu]** Mani, M., & Pillai, R. (2010). *Impact of dust on solar photovoltaic (PV) performance: Research status, challenges and recommendations*. Renewable and Sustainable Energy Reviews, 14(9), 3124–3131.
    * [Mã DOI Quốc tế](https://doi.org/10.1016/j.rser.2010.07.065)
    * *(Xác định dải tốc độ suy giảm hiệu suất do bám bụi $0{,}1\% - 0{,}2\%/\text{ngày}$ dùng làm tham số tính toán chu kỳ kinh tế $N^*$).*
...
| `Ngưỡng tự làm sạch 5mm [10]` | **[10] Kimber et al. (IEEE/NREL, 2006)** | Cung cấp bằng chứng thực nghiệm về ngưỡng mưa $5\,\text{mm}$ giúp phục hồi $95\%$ độ truyền quang bề mặt. |
| `Trạng thái làm sạch dữ liệu [11]` | **[11] Coello & Boyle (IEEE J-PV, 2019)** | Thuật toán xác định chuỗi ngày khô và trạng thái phục hồi quang học của tấm pin từ dữ liệu mưa. |
| `Hệ số bám bụi k_soiling [12]` | **[12] Mani & Pillai (Elsevier, 2010)** | Căn cứ khoa học của hệ số tốc độ bám bụi $0{,}15\%/\text{ngày}$ dùng để tính chu kỳ kinh tế $N^* = 21\,\text{ngày}$. |────────────┐
...
* **Cơ chế:** Khi góc nghiêng đạt $\ge 10^\circ - 15^\circ$, các trận mưa rào $\ge 10\,\text{mm}$ tạo màng nước chảy xiết cuốn trôi $95\% - 98\%$ bụi bẩn. Ngược lại, trên mái bằng $<8^\circ$, lực căng bề mặt giữ nước lại ở gờ nhôm đáy tạo thành dải bùn đọng (mud damming).
* **Định lượng 3 nguồn lợi ích tài chính & vận hành:**
  1. **Tiết kiệm chi phí nhân công rửa pin thủ công:** Mái bằng bắt buộc phải thuê đơn vị rửa $3 - 4$ lần/năm ($1.500\,\text{AUD}$/lần $\implies 4.500 - 6.000\,\text{AUD/năm}$). Khi nghiêng $15^\circ$, chỉ cần rửa $1$ lần/năm vào cuối mùa khô hạn $\implies$ **Tiết kiệm trực tiếp $3.500 - 4.500\,\text{AUD/năm}$**.
  2. **Thu hồi tổn thất do đọng bùn che bóng viền đáy:** Vệt bùn đọng che hàng cell dưới làm kích hoạt Bypass Diode, làm mất $33\%$ công suất chuỗi pin của $15\% - 20\%$ tấm pin mái bằng ($18.500\,\text{kWh/năm}$) $\implies$ Góc nghiêng $15^\circ$ thu hồi trọn vẹn **$18.500\,\text{kWh/năm} \implies +3.700\,\text{AUD/năm}$**.
...
* **P-type PERC cũ:** $\gamma = -0{,}38\%/^\circ\text{C}$, hiệu suất $\eta = 17{,}5\% - 19{,}5\%$, suy thoái $0{,}55\%/\text{năm}$, có LID.
* **N-type TOPCon mới:** $\gamma = -0{,}30\%/^\circ\text{C}$, hiệu suất $\eta = 22{,}0\% - 23{,}2\%$, suy thoái $0{,}40\%/\text{năm}$, Zero LID.
* **Sản lượng tăng thêm trên cùng diện tích mái:** Tăng **$+6{,}2\%$ tổng sản lượng năm** ($\mathbf{213.761\,\text{kWh/năm}}$).
...
* **Tấm chắn nắng Inverter ngoài trời:** Hạ nhiệt heatsink $<72^\circ\text{C}$, triệt tiêu lỗi quá nhiệt, thu hồi **$18.450\,\text{kWh/năm}$**, tránh hỏng sớm 2 bộ biến tần ($16.000\,\text{AUD}$).
* **DC Optimizers cho 6 trạm bóng che ($320\,\text{kWp}$):** Thu hồi **$38.624\,\text{kWh/năm}$**.
* **Tổng điện thu hồi:** $\mathbf{57.074\,\text{kWh/năm}}$ ($4.756\,\text{kWh/tháng}$).
* **Chi phí đầu tư CapEx:** $\mathbf{12.500\,\text{AUD}}$.
